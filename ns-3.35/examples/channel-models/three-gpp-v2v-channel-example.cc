 ...
 */
static void
ComputeSnr (Ptr<MobilityModel> txMob, Ptr<MobilityModel> rxMob, Ptr<const SpectrumValue> txPsd,
            double noiseFigure)
{
  ...
  *(rxPsd) *= propagationGainLinear;
  ...
  f << Simulator::Now ().GetSeconds () << " " // time [s]
    << txMob->GetPosition ().x << " " << txMob->GetPosition ().y << " " << rxMob->GetPosition ().x
    << " " << rxMob->GetPosition ().y << " " << cond->GetLosCondition () << " " // channel state
