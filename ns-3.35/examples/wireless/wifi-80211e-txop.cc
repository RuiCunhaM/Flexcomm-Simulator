...
main (int argc, char *argv[])
{
  ...
  WifiHelper
      wifi; //the default standard of 802.11a will be selected by this helper since the program doesn't specify another one
  ...
  std::cout << "  Maximum TXOP duration = " << beTxopTracer.m_max.GetMicroSeconds () << " us"
            << '\n';
  if (verifyResults &&
      (beTxopTracer.m_max < MicroSeconds (2700) || beTxopTracer.m_max > MicroSeconds (3008)))
    {
  ...
  std::cout << "  Maximum TXOP duration = " << viTxopTracer.m_max.GetMicroSeconds () << " us"
            << '\n';
  if (verifyResults &&
      (viTxopTracer.m_max < MicroSeconds (2700) || viTxopTracer.m_max > MicroSeconds (3008)))
    {
