  ...
  for (uint32_t i = 0; i < modes.size (); i++)
    {
      ...
      for (double snr = -10.0; snr <= 20.0; snr += 0.1)
        {
          double psYans = yans->GetChunkSuccessRate (WifiMode (modes[i]), txVector,
          ...
          double psNist = nist->GetChunkSuccessRate (WifiMode (modes[i]), txVector,
                                                     std::pow (10.0, snr / 10.0), FrameSize * 8);
          ...
          double psTable = table->GetChunkSuccessRate (WifiMode (modes[i]), txVector,
                                                       std::pow (10.0, snr / 10.0), FrameSize * 8);
