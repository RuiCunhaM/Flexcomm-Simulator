...
WaveBsmHelper::WaveBsmHelper () : m_waveBsmStats ()
{
  ...
  for (aci = bsmApps.Begin (); aci != bsmApps.End (); ++aci)
    {
      ...
      bsmApp->Setup (i, nodeId, totalTime, wavePacketSize, waveInterval, gpsAccuracyNs,
                     m_txSafetyRangesSq, GetWaveBsmStats (), &nodesMoving, chAccessMode,
                     txMaxDelay);
      nodeId++;
    }
}
