...
HtPhy::CreateHtMcs (uint8_t index)
{
  ...
  return WifiModeFactory::CreateWifiMcs (
      "HtMcs" + std::to_string (index), index, WIFI_MOD_CLASS_HT,
...
HtPhy::GetDataRate (uint8_t mcsValue, uint16_t channelWidth, uint16_t guardInterval, uint8_t nss)
{
  ...
  NS_ASSERT (nss <= 4);
  return CalculateDataRate (3.2, guardInterval, GetUsableSubcarriers (channelWidth),
                            static_cast<uint16_t> (log2 (GetHtConstellationSize (mcsValue))),
                            GetCodeRatio (GetHtCodeRate (mcsValue)), nss);
}

uint64_t
HtPhy::CalculateDataRate (double symbolDuration, uint16_t guardInterval, uint16_t usableSubCarriers,
                          uint16_t numberOfBitsPerSubcarrier, double codingRate, uint8_t nss)
{
  return nss * OfdmPhy::CalculateDataRate (symbolDuration, guardInterval, usableSubCarriers,
