                                                 ...
                                                 Mac48Address originator, uint16_t startingSeq)
{
  ...
  RecipientBlockAckAgreement agreement (
      originator, respHdr->IsAmsduSupported (), tid, respHdr->GetBufferSize () + 1,
  ...
  GetBaManager (tid)->SetBlockAckInactivityCallback (
                                       ...
                                       bool initialFrame)
{
  ...
  Ptr<WifiMacQueueItem> mpdu =
      edca->GetNextMpdu (peekedItem, txParams, availableTime, initialFrame, queueIt);
  ...
  std::vector<Ptr<WifiMacQueueItem>> mpduList =
      m_mpduAggregator->GetNextAmpdu (mpdu, txParams, availableTime, queueIt);
...
HtFrameExchangeManager::TransmissionSucceeded (void)
{
  ...
  if (m_edca != 0 && m_edca->GetTxopLimit ().IsZero () &&
      m_edca->GetBaManager ()->GetBar (false) != 0)
    {
  ...
  if (hdr.IsCtl ())
    {
      if (hdr.IsCts () && m_txTimer.IsRunning () &&
          m_txTimer.GetReason () == WifiTxTimer::WAIT_CTS && m_psdu != 0)
        {
