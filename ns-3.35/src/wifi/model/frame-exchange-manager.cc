...
FrameExchangeManager::FrameExchangeManager ()
    : m_navEnd (Seconds (0)), m_promisc (false), m_moreFragments (false)
{
...
FrameExchangeManager::SendMpdu (void)
{
  ...
  Time txDuration =
      m_phy->CalculateTxDuration (m_mpdu->GetSize (), m_txParams.m_txVector, m_phy->GetPhyBand ());
  ...
  if (m_txParams.m_acknowledgment->method == WifiAcknowledgment::NONE)
  ...
  else if (m_txParams.m_acknowledgment->method == WifiAcknowledgment::NORMAL_ACK)
    {
      m_mpdu->GetHeader ().SetDuration (GetFrameDurationId (
          m_mpdu->GetHeader (), m_mpdu->GetSize (), m_txParams, m_fragmentedPacket));
  ...
  if (m_moreFragments)
    {
      ...
      Simulator::Schedule (m_phy->GetSifs (), &FrameExchangeManager::StartTransmission, this,
                           m_dcf);
...
FrameExchangeManager::NotifyInternalCollision (Ptr<Txop> txop)
{
  ...
  Ptr<const WifiMacQueueItem> mpdu =
      (qosTxop != nullptr ? qosTxop->PeekNextMpdu () : txop->GetWifiMacQueue ()->Peek ());
          ...
          if (m_navEnd <= Simulator::Now ())
          ...
          else
            {
              NS_LOG_DEBUG ("Received RTS from=" << hdr.GetAddr2 () << ", cannot schedule CTS");
               ...
               m_txTimer.GetReason () == WifiTxTimer::WAIT_CTS && m_mpdu != 0)
        {
