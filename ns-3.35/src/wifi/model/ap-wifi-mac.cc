          ...
          if (m_stationManager->GetHtSupported (sta.second))
            {
              ...
              for (uint8_t j = 0;
                   j < (std::min (nMcs, m_stationManager->GetNMcsSupported (sta.second))); j++)
                {
                  WifiMode mcs = *itMcs++;
                  uint8_t nss = (mcs.GetMcsValue () / 8) + 1;
                  NS_ASSERT (nss > 0 && nss < 5);
                  uint64_t dataRate = mcs.GetDataRate (
                      m_stationManager->GetChannelWidthSupported (sta.second),
                      m_stationManager->GetShortGuardIntervalSupported (sta.second) ? 400 : 800,
                      nss);
      ...
      if (!found)
        {
          ...
          if (m_stationManager->GetDsssSupported (to) &&
              !m_stationManager->GetErpOfdmSupported (to))
            {
              m_numNonErpStations++;
            }
...
void
ApWifiMac::TxFailed (uint8_t timeoutReason, Ptr<const WifiMacQueueItem> mpdu,
          ...
          if (hdr->IsAssocReq ())
            {
              ...
              if (problem)
              ...
              else
                {
                  ...
                  if (GetErpSupported () && m_stationManager->GetErpOfdmSupported (from) &&
                  ...
                  if (GetHtSupported ())
                    {
                      ExtendedCapabilities extendedCapabilities =
                          assocReq.GetExtendedCapabilities ();
              ...
              if (problem)
              ...
              else
                {
                  ...
                  if (GetHtSupported ())
                    {
                      ExtendedCapabilities extendedCapabilities =
                          reassocReq.GetExtendedCapabilities ();
                      //TODO: to be completed
                    }
                  if (GetHeSupported ())
                    {
                  ...
                  if (it->second == from)
                    {
                      ...
                      if (m_stationManager->GetDsssSupported (from) &&
                          !m_stationManager->GetErpOfdmSupported (from))
                        {
                          m_numNonErpStations--;
                        }
...
ApWifiMac::DoInitialize (void)
{
  ...
  if (m_enableBeaconGeneration)
    {
      if (m_enableBeaconJitter)
        {
          Time jitter = MicroSeconds (static_cast<int64_t> (
              m_beaconJitter->GetValue (0, 1) * (GetBeaconInterval ().GetMicroSeconds ())));
  ...
  NS_ABORT_IF (!TraceConnectWithoutContext ("MpduResponseTimeout",
                                            MakeCallback (&ApWifiMac::TxFailed, this)));
