...
WifiRemoteStationManager::AddSupportedMcs (Mac48Address address, WifiMode mcs)
{
  ...
  NS_ASSERT (!address.IsGroup ());
  WifiRemoteStationState *state = LookupState (address);
  for (WifiModeListIterator i = state->m_operationalMcsSet.begin ();
  ...
  if (!header.IsMgt () && address.IsGroup ())
    {
      ...
      v.SetGuardInterval (ConvertGuardIntervalToNanoSeconds (
          mode, DynamicCast<WifiNetDevice> (m_wifiPhy->GetDevice ())));
  ...
  if (header.IsMgt ())
    {
      ...
      txVector.SetGuardInterval (ConvertGuardIntervalToNanoSeconds (
          mgtMode, DynamicCast<WifiNetDevice> (m_wifiPhy->GetDevice ())));
    }
  else
    {
...
WifiRemoteStationManager::GetCtsToSelfTxVector (void)
{
  ...
  return WifiTxVector (defaultMode, GetDefaultTxPowerLevel (), defaultPreamble,
                       ConvertGuardIntervalToNanoSeconds (
                           defaultMode, DynamicCast<WifiNetDevice> (m_wifiPhy->GetDevice ())),
...
WifiRemoteStationManager::GetCtsTxVector (Mac48Address to, WifiMode rtsTxMode) const
{
  ...
  uint16_t ctsTxGuardInterval = ConvertGuardIntervalToNanoSeconds (
      ctsMode, DynamicCast<WifiNetDevice> (m_wifiPhy->GetDevice ()));
...
WifiRemoteStationManager::GetAckTxVector (Mac48Address to, const WifiTxVector &dataTxVector) const
{
  ...
  uint16_t ackTxGuardInterval = ConvertGuardIntervalToNanoSeconds (
      ackMode, DynamicCast<WifiNetDevice> (m_wifiPhy->GetDevice ()));
                                               ...
                                               const WifiTxVector &dataTxVector) const
{
  ...
  uint16_t blockAckTxGuardInterval = ConvertGuardIntervalToNanoSeconds (
      blockAckMode, DynamicCast<WifiNetDevice> (m_wifiPhy->GetDevice ()));
