  ...
  if (m_edca->GetTxopLimit ().IsStrictlyPositive ())
    {
      ...
      if (!m_edca->IsTxopStarted () || (backingOff && m_edca->GetRemainingTxop ().IsZero ()))
        {
          // starting a new TXOP
                                             ...
                                             bool initialFrame)
{
  ...
  Ptr<WifiMacQueueItem> item =
      edca->GetNextMpdu (mpdu, txParams, availableTime, initialFrame, queueIt);
                                           ...
                                           Time response) const
{
  ...
  // The TXOP holder may exceed the TXOP limit in some situations (Sec. 10.22.2.8
  // of 802.11-2016)
  return std::max (m_edca->GetRemainingTxop () - m_phy->CalculateTxDuration (GetRtsSize (),
                                                                             rtsTxVector,
                                                                             m_phy->GetPhyBand ()),
                   Seconds (0));
}
                                                 ...
                                                 Time response) const
{
  ...
  // The TXOP holder may exceed the TXOP limit in some situations (Sec. 10.22.2.8
  // of 802.11-2016)
  return std::max (m_edca->GetRemainingTxop () - m_phy->CalculateTxDuration (GetCtsSize (),
                                                                             ctsTxVector,
                                                                             m_phy->GetPhyBand ()),
                   Seconds (0));
}
