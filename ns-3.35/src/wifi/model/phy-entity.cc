...
PhyEntity::GetPhyHeaderSnrPer (WifiPpduField field, Ptr<Event> event) const
{
  ...
  return m_wifiPhy->m_interference.CalculatePhyHeaderSnrPer (
      event, measurementChannelWidth, m_wifiPhy->GetPrimaryBand (measurementChannelWidth), field);
}
  ...
  else
    {
      ...
      switch (status.actionIfFailure)
        ...
        case DROP:
          ...
          m_state->SwitchMaybeToCcaBusy (
              GetRemainingDurationAfterField (ppdu, field)); //keep in CCA busy state till the end
        //no break
        case IGNORE:
  ...
  switch (m_state->GetState ())
    ...
    case WifiPhyState::SWITCHING:
      ...
      DropPreambleEvent (ppdu, CHANNEL_SWITCHING, endRx,
                         m_wifiPhy->GetMeasurementChannelWidth (ppdu));
      break;
...
PhyEntity::StartReceivePayload (Ptr<Event> event)
{
  ...
  Time payloadDuration =
      event->GetPpdu ()->GetTxDuration () - CalculatePhyPreambleAndHeaderDuration (txVector);
...
PhyEntity::DoStartReceivePayload (Ptr<Event> event)
{
  ...
  m_endRxPayloadEvents.push_back (Simulator::Schedule (
      ppdu->GetTxDuration () - CalculatePhyPreambleAndHeaderDuration (event->GetTxVector ()),
  ...
  if (std::count (statusPerMpduIt->second.begin (), statusPerMpduIt->second.end (), true))
    {
      ...
      m_state->SwitchFromRxEndOk (Copy (psdu), rxSignalInfo, txVector, staId,
...
PhyEntity::EndPreambleDetectionPeriod (Ptr<Event> event)
{
  ...
  if ((!m_wifiPhy->m_preambleDetectionModel && maxRxPowerW > 0.0) ||
  ...
  else
    {
      ...
      DropPreambleEvent (
          m_wifiPhy->m_currentEvent->GetPpdu (), PREAMBLE_DETECT_FAILURE,
          m_wifiPhy->m_currentEvent->GetEndTime (),
          m_wifiPhy->GetMeasurementChannelWidth (m_wifiPhy->m_currentEvent->GetPpdu ()));
      if (m_wifiPhy->m_currentPreambleEvents.empty ())
        {
...
PhyEntity::GetRxPowerWForPpdu (Ptr<Event> event) const
{
  return event->GetRxPowerW (
      m_wifiPhy->GetPrimaryBand (GetMeasurementChannelWidth (event->GetPpdu ())));
}
