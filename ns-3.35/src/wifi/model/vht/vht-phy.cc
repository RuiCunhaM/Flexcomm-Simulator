...
VhtPhy::CreateVhtMcs (uint8_t index)
{
  ...
  return WifiModeFactory::CreateWifiMcs (
      "VhtMcs" + std::to_string (index), index, WIFI_MOD_CLASS_VHT,
      ...
      MakeBoundCallback (&GetNonHtReferenceRate, index), MakeBoundCallback (&IsModeAllowed, index));
}
...
VhtPhy::GetDataRate (uint8_t mcsValue, uint16_t channelWidth, uint16_t guardInterval, uint8_t nss)
{
  ...
  NS_ASSERT (nss <= 8);
  NS_ASSERT_MSG (IsModeAllowed (mcsValue, channelWidth, nss),
                 "VHT MCS " << +mcsValue << " forbidden at " << channelWidth << " MHz when NSS is "
                            << +nss);
  return HtPhy::CalculateDataRate (3.2, guardInterval, GetUsableSubcarriers (channelWidth),
