      ...
      if (!IsBusy ())
        {
          ...
          Time delay =
              (txop->IsQosTxop () ? Seconds (0) : GetSifs () + txop->GetAifsn () * GetSlot ());
...
ChannelAccessManager::GetAccessGrantStart (bool ignoreNav) const
{
  ...
  if (ignoreNav)
    {
      accessGrantedStart =
          MostRecent ({rxAccessStart, busyAccessStart, txAccessStart, ackTimeoutAccessStart,
                       ctsTimeoutAccessStart, switchingAccessStart});
    }
  else
    {
      accessGrantedStart =
          MostRecent ({rxAccessStart, busyAccessStart, txAccessStart, navAccessStart,
...
{
  NS_LOG_FUNCTION (this << txop);
  Time mostRecentEvent = MostRecent (
      {txop->GetBackoffStart (), GetAccessGrantStart () + (txop->GetAifsn () * GetSlot ())});
...
ChannelAccessManager::DisableEdcaFor (Ptr<Txop> qosTxop, Time duration)
{
  ...
  NS_LOG_DEBUG ("Backoff will resume at time " << resume << " with " << qosTxop->GetBackoffSlots ()
                                               << " remaining slot(s)");
