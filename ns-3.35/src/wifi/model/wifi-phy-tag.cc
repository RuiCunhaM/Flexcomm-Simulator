...
{
  static TypeId tid = TypeId ("ns3::WifiPhyTag").SetParent<Tag> ();
  return tid;
}
...
WifiPhyTag::WifiPhyTag (WifiPreamble preamble, WifiModulationClass modulation,
                        uint8_t frameComplete)
    : m_preamble (preamble), m_modulation (modulation), m_frameComplete (frameComplete)
{
}
