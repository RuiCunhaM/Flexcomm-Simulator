...
WifiPsdu::WifiPsdu (Ptr<const Packet> p, const WifiMacHeader &header) : m_isSingle (false)
{
  m_mpduList.push_back (Create<WifiMacQueueItem> (p, header));
  m_size = header.GetSerializedSize () + p->GetSize () + WIFI_MAC_FCS_LENGTH;
}

WifiPsdu::WifiPsdu (Ptr<WifiMacQueueItem> mpdu, bool isSingle) : m_isSingle (isSingle)
{
