...
{
  static TypeId tid = TypeId ("ns3::MultiUserScheduler").SetParent<Object> ().SetGroupName ("Wifi");
  return tid;
}
...
MultiUserScheduler::DoInitialize (void)
{
  ...
  for (uint8_t i = 0; i < 8; i++)
    {
      m_sizeOf8QosNull =
          MpduAggregator::GetSizeIfAggregated (headerSize + WIFI_MAC_FCS_LENGTH, m_sizeOf8QosNull);
    }
}
