...
HePhy::HePhy (bool buildModeList /* = true */) : VhtPhy (false) //don't add VHT modes to list
{
...
{
  NS_ABORT_MSG_IF (!txVector.IsDlMu () || (txVector.GetModulationClass () != WIFI_MOD_CLASS_HE),
                   "HE-SIG-B only available for HE MU");
  /**
                                            ...
                                            WifiPhyBand band)
{
  ...
  Time preambleDuration =
      WifiPhy::GetStaticPhyEntity (WIFI_MOD_CLASS_HE)
          ->CalculatePhyPreambleAndHeaderDuration (
              txVector); //this is quite convoluted but only way of keeping the method static
  uint8_t sigExtension = 0;
  if (band == WIFI_PHY_BAND_2_4GHZ)
    {
      sigExtension = 6;
                             ...
                             Time rxDuration)
{
  ...
  if (txVector.IsUlMu () && psdFlag == HePpdu::PSD_HE_TB_OFDMA_PORTION)
    {
      NS_ASSERT (txVector.GetModulationClass () == WIFI_MOD_CLASS_HE);
      if (m_currentHeTbPpduUid == ppdu->GetUid () && GetCurrentEvent () != 0)
        {
      ...
      else
        {
          ...
          NS_LOG_INFO ("Consider OFDMA part of the HE TB PPDU as interference since device dropped "
                       "the preamble");
...
HePhy::StartReceiveOfdmaPayload (Ptr<Event> event)
{
  ...
  m_signalNoiseMap.insert (
      {std::make_pair (ppdu->GetUid (), ppdu->GetStaId ()), SignalNoiseDbm ()});
  m_statusPerMpduMap.insert (
      {std::make_pair (ppdu->GetUid (), ppdu->GetStaId ()), std::vector<bool> ()});
}
  ...
  if (flag == HePpdu::PSD_HE_TB_OFDMA_PORTION)
    {
      ...
      v = WifiSpectrumValueHelper::CreateHeMuOfdmTxPowerSpectralDensity (
  ...
  else
    {
      ...
      v = WifiSpectrumValueHelper::CreateHeOfdmTxPowerSpectralDensity (
...
HePhy::CreateHeMcs (uint8_t index)
{
  ...
  return WifiModeFactory::CreateWifiMcs (
      "HeMcs" + std::to_string (index), index, WIFI_MOD_CLASS_HE,
      MakeBoundCallback (&GetCodeRate, index), MakeBoundCallback (&GetConstellationSize, index),
      MakeBoundCallback (&GetPhyRate, index), MakeCallback (&GetPhyRateFromTxVector),
      MakeBoundCallback (&GetDataRate, index), MakeCallback (&GetDataRateFromTxVector),
      MakeBoundCallback (&GetNonHtReferenceRate, index), MakeCallback (&IsModeAllowed));
}
...
HePhy::GetDataRate (uint8_t mcsValue, uint16_t channelWidth, uint16_t guardInterval, uint8_t nss)
{
  ...
  NS_ASSERT (nss <= 8);
  return HtPhy::CalculateDataRate (12.8, guardInterval, GetUsableSubcarriers (channelWidth),
