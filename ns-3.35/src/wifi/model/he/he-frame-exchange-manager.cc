...
{
  NS_ASSERT (m_mac);
  NS_ABORT_MSG_IF (m_apMac == 0, "A Multi-User Scheduler can only be aggregated to an AP");
                                            ...
                                            bool initialFrame)
{
   ...
   */
  if (m_muScheduler != 0 && edca->GetBaManager ()->GetBar (false) == nullptr &&
      (mpdu == 0 ||
  ...
  if (txFormat == MultiUserScheduler::UL_MU_TX)
    {
      if (m_muScheduler->GetUlMuInfo ().trigger == nullptr)
        {
          NS_LOG_DEBUG ("The Multi-user Scheduler returned UL_MU_TX with empty Trigger Frame, do "
                        "not transmit");
          return false;
        }
      ...
      SendPsduMapWithProtection (
          WifiPsduMap{{SU_STA_ID, GetWifiPsdu (m_muScheduler->GetUlMuInfo ().trigger,
      ...
      if (m_triggerFrame == nullptr)
      ...
      else
        {
          ...
          Time txDuration = m_phy->CalculateTxDuration (
              m_triggerFrame->GetSize (), acknowledgment->muBarTxVector, m_phy->GetPhyBand ());
   ...
   */
  else if (m_txParams.m_acknowledgment->method == WifiAcknowledgment::UL_MU_MULTI_STA_BA)
    {
      ...
      acknowledgment->acknowledgmentTime +=
          m_mac->GetWifiPhy ()->GetSifs () + m_muScheduler->GetUlMuInfo ().tbPpduDuration;
...
HeFrameExchangeManager::CalculateAcknowledgmentTime (WifiAcknowledgment *acknowledgment) const
{
   ...
   */
  else if (acknowledgment->method == WifiAcknowledgment::DL_MU_TF_MU_BAR)
    {
      ...
      dlMuTfMuBarAcknowledgment->ulLength =
          HePhy::ConvertHeTbPpduDurationToLSigLength (duration, m_phy->GetPhyBand ());
      WifiTxVector &txVector =
   ...
   */
  else if (acknowledgment->method == WifiAcknowledgment::DL_MU_AGGREGATE_TF)
    {
      ...
      // The computed duration may not be coded exactly in the L-SIG length, hence determine
      // the exact duration corresponding to the value that will be coded in this field.
      dlMuAggrTfAcknowledgment->ulLength =
          HePhy::ConvertHeTbPpduDurationToLSigLength (duration, m_phy->GetPhyBand ());
      WifiTxVector &txVector =
      ...
      if ((mpdu = edca->PeekNextMpdu (tid, hdr.GetAddr2 ())) != 0)
        {
          ...
          Ptr<WifiMacQueueItem> item =
              edca->GetNextMpdu (mpdu, txParams, ppduDuration, false, queueIt);

          if (item != 0)
            {
              // try A-MPDU aggregation
              std::vector<Ptr<WifiMacQueueItem>> mpduList =
                  m_mpduAggregator->GetNextAmpdu (item, txParams, ppduDuration, queueIt);
