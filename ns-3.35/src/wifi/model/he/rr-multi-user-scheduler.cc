...
RrMultiUserScheduler::RrMultiUserScheduler () : m_ulTriggerType (TriggerFrameType::BASIC_TRIGGER)
{
...
RrMultiUserScheduler::SelectTxFormat (void)
{
  ...
  if (m_enableUlOfdma &&
      (GetLastTxFormat () == DL_MU_TX || m_ulTriggerType == TriggerFrameType::BSRP_TRIGGER))
    {
...
RrMultiUserScheduler::TrySendingBsrpTf (void)
{
  ...
  if (trigger.GetNUserInfoFields () == 1)
    {
      NS_ASSERT (m_apMac->GetStaList ().find (trigger.begin ()->GetAid12 ()) !=
                 m_apMac->GetStaList ().end ());
  ...
  for (const auto &userInfo : trigger)
    {
      Time duration = WifiPhy::CalculateTxDuration (
          m_sizeOf8QosNull, txVector, m_apMac->GetWifiPhy ()->GetPhyBand (), userInfo.GetAid12 ());
  ...
  trigger.SetUlLength (HePhy::ConvertHeTbPpduDurationToLSigLength (
      qosNullTxDuration, m_apMac->GetWifiPhy ()->GetPhyBand ()));
  ...
  for (const auto &candidate : m_candidates)
    {
      ...
      if (queueSize == 255)
        ...
        {
          NS_LOG_DEBUG ("Buffer status of station " << candidate.first->address
                                                    << " is not limited");
          maxBufferSize = 0xffffffff;
        }
      else
        {
          NS_LOG_DEBUG ("Buffer status of station " << candidate.first->address << " is "
                                                    << +queueSize);
  ...
  if (maxBufferSize > 0)
    {
      ...
      HeRu::RuType ruType = HeRu::GetEqualSizedRusForStations (
          m_apMac->GetWifiPhy ()->GetChannelWidth (), count, nCentral26TonesRus);
          ...
          for (std::size_t i = 0; i < count + nCentral26TonesRus; i++)
            {
              ...
              txVector.SetHeMuUserInfo (staId, {{(i < count ? ruType : HeRu::RU_26_TONE), 1, false},
      ...
      trigger.SetUlLength (HePhy::ConvertHeTbPpduDurationToLSigLength (
          maxDuration, m_apMac->GetWifiPhy ()->GetPhyBand ()));
      trigger.SetCsRequired (true);
      m_heFem->SetTargetRssi (trigger);
...
RrMultiUserScheduler::TrySendingDlMuPpdu (void)
{
  ...
  std::size_t count =
      std::min (static_cast<std::size_t> (m_nStations), m_staList[primaryAc].size ());
  std::size_t nCentral26TonesRus;
  HeRu::RuType ruType = HeRu::GetEqualSizedRusForStations (
      m_apMac->GetWifiPhy ()->GetChannelWidth (), count, nCentral26TonesRus);
  NS_ASSERT (count >= 1);
...
RrMultiUserScheduler::ComputeDlMuInfo (void)
{
  ...
  for (std::size_t i = 0; i < nRusAssigned + nCentral26TonesRus; i++)
    {
      ...
      // AssignRuIndices will be called below to set RuSpec
      dlMuInfo.txParams.m_txVector.SetHeMuUserInfo (
          staId, {{(i < nRusAssigned ? ruType : HeRu::RU_26_TONE), 1, false},
  ...
  for (const auto &candidate : m_candidates)
    {
      ...
      std::vector<Ptr<WifiMacQueueItem>> mpduList = m_heFem->GetMpduAggregator ()->GetNextAmpdu (
          item, dlMuInfo.txParams, m_availableTime, queueIt);
  ...
  double creditsPerSta =
      dlMuInfo.txParams.m_txDuration.ToDouble (Time::US) / m_staList[primaryAc].size ();
  ...
  for (std::size_t i = 0; i < nRusAssigned + nCentral26TonesRus; i++)
    {
      ...
      candidateIt->first->credits -=
          debitsPerMhz * HeRu::GetBandwidth (i < nRusAssigned ? ruType : HeRu::RU_26_TONE);

      candidateIt++;
    }

  // sort the list in decreasing order of credits
  m_staList[primaryAc].sort (
      [] (const MasterInfo &a, const MasterInfo &b) { return a.credits > b.credits; });
