                                    ...
                                    WifiMode mode, WifiPreamble preamble, uint8_t txLevel)
{
  ...
  *stream->GetStream () << "t " << Simulator::Now ().GetSeconds () << " " << mode << " " << *p
                        << std::endl;
}
                                   ...
                                   WifiMode mode, WifiPreamble preamble)
{
  ...
  *stream->GetStream () << "r " << Simulator::Now ().GetSeconds () << " " << mode << " " << *p
                        << std::endl;
}

WifiPhyHelper::WifiPhyHelper () : m_pcapDlt (PcapHelper::DLT_IEEE802_11)
{
...
void
WifiPhyHelper::SetErrorRateModel (std::string name, std::string n0, const AttributeValue &v0,
...
void
WifiPhyHelper::SetFrameCaptureModel (std::string name, std::string n0, const AttributeValue &v0,
                                     std::string n1, const AttributeValue &v1, std::string n2,
                                     const AttributeValue &v2, std::string n3,
                                     const AttributeValue &v3, std::string n4,
                                     const AttributeValue &v4, std::string n5,
...
void
WifiPhyHelper::SetPreambleDetectionModel (
    std::string name, std::string n0, const AttributeValue &v0, std::string n1,
    const AttributeValue &v1, std::string n2, const AttributeValue &v2, std::string n3,
    const AttributeValue &v3, std::string n4, const AttributeValue &v4, std::string n5,
    const AttributeValue &v5, std::string n6, const AttributeValue &v6, std::string n7,
    const AttributeValue &v7)
{
...
WifiHelper::WifiHelper ()
    : m_standard (WIFI_STANDARD_80211a), m_selectQueueCallback (&SelectQueueByDSField)
{
  SetRemoteStationManager ("ns3::ArfWifiManager");
}

void
WifiHelper::SetRemoteStationManager (std::string type, std::string n0, const AttributeValue &v0,
                                     std::string n1, const AttributeValue &v1, std::string n2,
                                     const AttributeValue &v2, std::string n3,
                                     const AttributeValue &v3, std::string n4,
                                     const AttributeValue &v4, std::string n5,
...
void
WifiHelper::SetObssPdAlgorithm (std::string type, std::string n0, const AttributeValue &v0,
                                std::string n1, const AttributeValue &v1, std::string n2,
  ...
  for (NodeContainer::Iterator i = first; i != last; ++i)
    {
      ...
      if ((it->second.phyStandard >= WIFI_PHY_STANDARD_80211ac) &&
          (it->second.phyBand != WIFI_PHY_BAND_2_4GHZ))
        {
