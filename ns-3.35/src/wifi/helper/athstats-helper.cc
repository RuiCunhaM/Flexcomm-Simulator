...
AthstatsWifiTraceSink::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::AthstatsWifiTraceSink")
          ...
          .AddAttribute ("Interval", "Time interval between reports", TimeValue (Seconds (1.0)),
                         MakeTimeAccessor (&AthstatsWifiTraceSink::m_interval), MakeTimeChecker ());
  return tid;
}
...
void
AthstatsWifiTraceSink::PhyRxOkTrace (std::string context, Ptr<const Packet> packet, double snr,
...
void
AthstatsWifiTraceSink::PhyTxTrace (std::string context, Ptr<const Packet> packet, WifiMode mode,
...
void
AthstatsWifiTraceSink::PhyStateTrace (std::string context, Time start, Time duration,
                                      WifiPhyState state)
{
...
AthstatsWifiTraceSink::Open (std::string const &name)
{
  ...
  NS_ABORT_MSG_IF (m_writer->fail (), "AthstatsWifiTraceSink::Open (): m_writer->open ("
                                          << name.c_str () << ") failed");
...
AthstatsWifiTraceSink::WriteStats ()
{
  ...
  char str[200];
  snprintf (
      str, 200, "%8u %8u %7u %7u %7u %6u %6u %6u %7u %4u %3uM\n",
      (unsigned int)
          m_txCount, // /proc/net/dev transmitted packets to which we should subtract management frames
      (unsigned int)
