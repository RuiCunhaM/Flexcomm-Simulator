...
TestMultiUserScheduler::SelectTxFormat (void)
{
  ...
  if (m_txFormat == SU_TX || m_txFormat == DL_MU_TX ||
      (m_txFormat == UL_MU_TX && m_ulTriggerType == TriggerFrameType::BSRP_TRIGGER))
    {
      ...
      uint32_t ampduSize =
          (ulTriggerType == TriggerFrameType::BSRP_TRIGGER ? m_sizeOf8QosNull : 3500);

      Time duration =
          WifiPhy::CalculateTxDuration (ampduSize, txVector, m_apMac->GetWifiPhy ()->GetPhyBand (),
                                        m_apMac->GetStaList ().begin ()->first);

      uint16_t length = HePhy::ConvertHeTbPpduDurationToLSigLength (
          duration, m_apMac->GetWifiPhy ()->GetPhyBand ());
      ...
      m_txParams.m_txVector =
          m_apMac->GetWifiRemoteStationManager ()->GetRtsTxVector (hdr.GetAddr1 ());
      ...
      for (auto &sta : staList)
        {
          ...
          Ptr<WifiMacQueueItem> mpdu = m_apMac->GetQosTxop (AC_BE)->GetNextMpdu (
              peeked, m_txParams, m_availableTime, m_initialFrame, queueIt);
          if (mpdu == 0)
            {
  ...
  if (txVector.GetPreambleType () == WIFI_PREAMBLE_HE_MU)
    {
      ...
      Ptr<WifiMacQueue> queue =
          DynamicCast<RegularWifiMac> (dev->GetMac ())->GetQosTxop (AC_BE)->GetWifiMacQueue ();
      m_flushed = 0;
      ...
      for (uint32_t i = 0; i < m_staDevices.GetN (); i++)
        {
          ...
          if (dev->GetAddress () == sender)
            {
              Ptr<QosTxop> qosTxop =
                  DynamicCast<RegularWifiMac> (dev->GetMac ())->GetQosTxop (AC_BE);
  ...
  for (uint8_t i = 0; i < 8; i++)
    {
      ...
      if (tid == 0)
        {
          NS_TEST_EXPECT_MSG_GT (hdr.GetQosQueueSize (), 0,
                                 "Expected a non null queue size for TID " << +tid);
        }
      else
        {
          NS_TEST_EXPECT_MSG_EQ (hdr.GetQosQueueSize (), 0,
                                 "Expected a null queue size for TID " << +tid);
        }
    }
          ...
          if (m_txPsdus[i].psduMap.size () == 1 &&
              ...
              !m_txPsdus[i].txVector.IsUlMu ())
            {
              NS_TEST_EXPECT_MSG_GT_OR_EQ (
                  ...
                  m_edcaDisabledStartTime.GetMicroSeconds () + m_muEdcaParameterSet.muTimer * 8192,
...
OfdmaAckSequenceTest::DoRun (void)
{
  ...
  CheckResults (dev->GetMac ()->GetWifiPhy ()->GetSifs (),
                dev->GetMac ()->GetWifiPhy ()->GetSlot (), apBeQosTxop->GetAifsn ());
