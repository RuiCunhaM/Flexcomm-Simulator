  ...
  if (psdu->GetNMpdus () == 1 && psdu->GetHeader (0).IsQosData () && txVector.IsUlMu ())
    {
      ...
      NS_LOG_INFO ("RECEIVED FROM BSS=" << +bss << " STA=" << +station << "  " << *psdu);
      ...
      for (uint8_t i = 0; i < m_nStationsPerBss; i++)
        {
          ...
          Simulator::Schedule (
              ...
              MakeCallback (&WifiPrimaryChannelsTest::ReceiveDl, this).TwoBind (bss, i));
        }
      auto dev = DynamicCast<WifiNetDevice> (m_apDevices.Get (bss));
      ...
      if (duration.IsZero ())
        {
          ...
          length = HePhy::ConvertHeTbPpduDurationToLSigLength (
              duration, staDev->GetMac ()->GetWifiPhy ()->GetPhyBand ());
        }
