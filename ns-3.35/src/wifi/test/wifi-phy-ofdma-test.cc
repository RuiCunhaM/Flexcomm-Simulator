...
TestUlOfdmaPpduUid::SendTbPpdu (void)
{
  ...
  txVector1.SetLength (
      HePhy::ConvertHeTbPpduDurationToLSigLength (txDuration, m_phySta1->GetPhyBand ()));
  txVector2.SetLength (
      HePhy::ConvertHeTbPpduDurationToLSigLength (txDuration, m_phySta2->GetPhyBand ()));
...
TestUlOfdmaPhyTransmission::VerifyEventsCleared (void)
{
  ...
  NS_TEST_ASSERT_MSG_EQ (m_phySta1->GetCurrentEvent (), 0,
                         "m_currentEvent for STA 1 was not cleared");
  NS_TEST_ASSERT_MSG_EQ (m_phySta2->GetCurrentEvent (), 0,
...
TestUlOfdmaPhyTransmission::ScheduleTest (
    ...
    uint32_t expectedBytesFromSta2, bool scheduleTxSta1, WifiPhyState expectedStateBeforeEnd)
{
  ...
  Simulator::Schedule (delay - MilliSeconds (10), &TestUlOfdmaPhyTransmission::SendHeSuPpdu, this,
                       0, 50, solicited ? 0 : 2, 0);
  //STA1 and STA2 send MU UL PPDUs addressed to AP
  if (scheduleTxSta1)
    {
      Simulator::Schedule (delay, &TestUlOfdmaPhyTransmission::SendHeTbPpdu, this, 1, 1, 1000, 0,
                           0);
    }
  ...
  Simulator::Schedule (delay + m_expectedPpduDuration, &TestUlOfdmaPhyTransmission::CheckPhyState,
...
TestUlOfdmaPhyTransmission::RunOne (void)
{
  ...
  ScheduleTest (delay, false, WifiPhyState::IDLE, 1, 0,
                1000, //One PSDU of 1000 bytes should have been successfully received from STA 1
                1, 0,
                1001); //One PSDU of 1001 bytes should have been successfully received from STA 2
  delay += Seconds (1.0);
  ...
  *interferencePsdRu1 = interferencePower / ((m_channelWidth / 2) * 20e6);
  ...
  ScheduleTest (
      delay, false,
      WifiPhyState::
          CCA_BUSY, //PHY should move to CCA_BUSY instead of IDLE due to the interference (primary channel)
      0, 1,
      0, //Reception of the PSDU from STA 1 should have failed (since interference occupies RU 1)
      1, 0, 1001); //One PSDU of 1001 bytes should have been successfully received from STA 2
  delay += Seconds (1.0);
  ...
  *interferencePsdRu2 = interferencePower / ((m_channelWidth / 2) * 20e6);
  ...
  ScheduleTest (
      delay, true,
      WifiPhyState::
          CCA_BUSY, //PHY should move to CCA_BUSY since measurement channel encompasses total channel width
  ...
  ScheduleTest (
      ...
      (m_channelWidth >= 40)
          ? WifiPhyState::IDLE
          : WifiPhyState::
                CCA_BUSY, //PHY should move to CCA_BUSY instead of IDLE if interference on primary channel
      1, 0, 1000, //One PSDU of 1000 bytes should have been successfully received from STA 1
      0, 1,
      0); //Reception of the PSDU from STA 2 should have failed (since interference occupies RU 2)
  delay += Seconds (1.0);
  ...
  *interferencePsdAll = interferencePower / (m_channelWidth * 20e6);
  ...
  Simulator::Schedule (delay, &TestUlOfdmaPhyTransmission::LogScenario, this,
                       "Reception of unsolicited HE TB PPDUs with interference on the full band "
                       "during PSDU reception");
  //A strong non-wifi interference is generated on the full band during PSDU reception
  Simulator::Schedule (delay + MicroSeconds (50), &TestUlOfdmaPhyTransmission::GenerateInterference,
                       this, interferencePsdAll, MilliSeconds (100));
  ...
  Simulator::Schedule (delay, &TestUlOfdmaPhyTransmission::LogScenario, this,
                       "Reception of unsolicited HE TB PPDUs with another HE TB PPDU arriving on "
  ...
  ScheduleTest (
      delay, false,
      WifiPhyState::
          CCA_BUSY, //PHY should move to CCA_BUSY instead of IDLE due to the interference on primary channel width
      0, 1,
      0, //Reception of the PSDU from STA 1 should have failed (since interference from STA 3 on same 20 MHz channel)
  ...
  Simulator::Schedule (delay + NanoSeconds (300), &TestUlOfdmaPhyTransmission::SendHeSuPpdu, this,
                       3, 1002, 1, 0);
  ScheduleTest (
      delay, false, WifiPhyState::IDLE, 0, 1,
      0, //Reception of the PSDU from STA 1 should have failed failed (since interference from STA 3)
      0, 1,
      0); //Reception of the PSDU from STA 2 should have failed failed (since interference from STA 3)
  delay += Seconds (1.0);
  ...
  Simulator::Schedule (
      ...
      (m_channelWidth >= 40)
          ? WifiPhyState::IDLE
          : WifiPhyState::
                RX); //PHY should move to RX instead of IDLE if HE TB PPDU on primary channel;
  ...
  ScheduleTest (delay, false, WifiPhyState::IDLE, 0, 0, 0, //No transmission scheduled for STA 1
                succ, fail, bytes, false, state);
  delay += Seconds (1.0);
...
TestUlOfdmaPowerControl::SendMuBar (std::vector<uint16_t> staIds)
{
  ...
  muBar.SetUlLength (
      HePhy::ConvertHeTbPpduDurationToLSigLength (MicroSeconds (128), WIFI_PHY_BAND_5GHZ));
  ...
  nav += m_phyAp->CalculateTxDuration (GetBlockAckSize (BlockAckType::COMPRESSED),
                                       muBar.GetHeTbTxVector (staId), DEFAULT_WIFI_BAND, staId);
