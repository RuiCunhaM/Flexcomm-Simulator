...
public:
   ...
   */
  WifiOfdmMaskSlopesTestCase (const char *str, WifiPhyStandard standard, WifiPhyBand band,
                              uint8_t bw, IndexPowerVect maskRefsLeft, IndexPowerVect maskRefsRight,
                              double tol);
...
private:
   ...
   * \param tol tolerance (in dB)
  */
  static void InterpolateAndAppendValues (IndexPowerVect &vect, IndexPowerPair start,
                                          IndexPowerPair stop, double tol);
};

WifiOfdmMaskSlopesTestCase::WifiOfdmMaskSlopesTestCase (const char *str, WifiPhyStandard standard,
                                                        WifiPhyBand band, uint8_t bw,
                                                        IndexPowerVect maskRefsLeft,
                                                        IndexPowerVect maskRefsRight, double tol)
    : TestCase (std::string ("SpectrumValue ") + str)
{
  NS_LOG_FUNCTION (this << str << standard << band << +bw << tol);
  NS_ASSERT (maskRefsLeft.size () % 2 == 0 &&
             maskRefsRight.size () % 2 == 0); //start/stop pairs expected
  ...
  switch (standard)
    ...
    case WIFI_PHY_STANDARD_80211p:
      ...
      m_actualSpectrum = WifiSpectrumValueHelper::CreateOfdmTxPowerSpectralDensity (
          freq, bw, refTxPowerW, bw, -20.0, -28.0, outerBandMaximumRejection);
      break;
  ...
  NS_ASSERT (m_expectedLeftPsd.size () ==
             (m_expectedLeftPsd.back ().first - m_expectedLeftPsd.front ().first + 1));
  ...
  NS_ASSERT (m_expectedRightPsd.size () ==
             (m_expectedRightPsd.back ().first - m_expectedRightPsd.front ().first + 1));
}
...
WifiOfdmMaskSlopesTestCase::DoRun (void)
{
  ...
  for (Values::const_iterator vit = m_actualSpectrum->ConstValuesBegin ();
  ...
  for (IndexPowerVect::const_iterator it = m_expectedLeftPsd.begin ();
       it != m_expectedLeftPsd.end (); it++)
    {
      currentPowerDbr = 10.0 * std::log10 ((*m_actualSpectrum)[it->first] / maxPowerW);
      NS_LOG_LOGIC ("For " << it->first << ", expected: " << it->second
  ...
  for (IndexPowerVect::const_iterator it = m_expectedRightPsd.begin ();
       it != m_expectedRightPsd.end (); it++)
    {
      currentPowerDbr = 10.0 * std::log10 ((*m_actualSpectrum)[it->first] / maxPowerW);
      NS_LOG_LOGIC ("For " << it->first << ", expected: " << it->second
                           << " vs obtained: " << currentPowerDbr);
...
WifiTransmitMaskTestSuite::WifiTransmitMaskTestSuite () : TestSuite ("wifi-transmit-mask", UNIT)
{
  ...
  AddTestCase (new WifiOfdmMaskSlopesTestCase ("11p 5MHz", WIFI_PHY_STANDARD_80211p,
                                               WIFI_PHY_BAND_5GHZ, 5, maskSlopesLeft,
                                               maskSlopesRight, tol),
               TestCase::QUICK);
  ...
  maskSlopesRight.push_back (std::make_pair (161, -28.375)); // Outer band right (start)
  maskSlopesRight.push_back (std::make_pair (192, -40.0)); // Outer band right (stop)
  AddTestCase (new WifiOfdmMaskSlopesTestCase ("11p 10MHz", WIFI_PHY_STANDARD_80211p,
                                               WIFI_PHY_BAND_5GHZ, 10, maskSlopesLeft,
                                               maskSlopesRight, tol),
               TestCase::QUICK);
  ...
  // same slppes as 11g
  AddTestCase (new WifiOfdmMaskSlopesTestCase ("11g", WIFI_PHY_STANDARD_80211g,
                                               WIFI_PHY_BAND_2_4GHZ, 20, maskSlopesLeft,
                                               maskSlopesRight, tol),
               TestCase::QUICK);
  ...
  AddTestCase (new WifiOfdmMaskSlopesTestCase ("11n_2.4GHz 20MHz", WIFI_PHY_STANDARD_80211n,
                                               WIFI_PHY_BAND_2_4GHZ, 20, maskSlopesLeft,
                                               maskSlopesRight, tol),
               TestCase::QUICK);
  ...
  maskSlopesRight.push_back (std::make_pair (161, -28.375)); // Outer band right (start)
  maskSlopesRight.push_back (std::make_pair (192, -40.0)); // Outer band right (stop)
  AddTestCase (new WifiOfdmMaskSlopesTestCase ("11n_5GHz 20MHz", WIFI_PHY_STANDARD_80211n,
                                               WIFI_PHY_BAND_5GHZ, 20, maskSlopesLeft,
                                               maskSlopesRight, tol),
               TestCase::QUICK);
  ...
  AddTestCase (new WifiOfdmMaskSlopesTestCase ("11n_2.4GHz 40MHz", WIFI_PHY_STANDARD_80211n,
                                               WIFI_PHY_BAND_2_4GHZ, 40, maskSlopesLeft,
                                               maskSlopesRight, tol),
               TestCase::QUICK);
  ...
  AddTestCase (new WifiOfdmMaskSlopesTestCase ("11n_5GHz 40MHz", WIFI_PHY_STANDARD_80211n,
                                               WIFI_PHY_BAND_5GHZ, 40, maskSlopesLeft,
                                               maskSlopesRight, tol),
               TestCase::QUICK);
  ...
  maskSlopesRight.push_back (std::make_pair (161, -28.375)); // Outer band right (start)
  maskSlopesRight.push_back (std::make_pair (192, -40.0)); // Outer band right (stop)
  AddTestCase (new WifiOfdmMaskSlopesTestCase ("11ac 20MHz", WIFI_PHY_STANDARD_80211ac,
                                               WIFI_PHY_BAND_5GHZ, 20, maskSlopesLeft,
                                               maskSlopesRight, tol),
               TestCase::QUICK);
  ...
  maskSlopesRight.push_back (std::make_pair (321, -28.188)); // Outer band right (start)
  maskSlopesRight.push_back (std::make_pair (384, -40.0)); // Outer band right (stop)
  AddTestCase (new WifiOfdmMaskSlopesTestCase ("11ac 40MHz", WIFI_PHY_STANDARD_80211ac,
                                               WIFI_PHY_BAND_5GHZ, 40, maskSlopesLeft,
                                               maskSlopesRight, tol),
               TestCase::QUICK);
  ...
  AddTestCase (new WifiOfdmMaskSlopesTestCase ("11ac 80MHz", WIFI_PHY_STANDARD_80211ac,
                                               WIFI_PHY_BAND_5GHZ, 80, maskSlopesLeft,
                                               maskSlopesRight, tol),
               TestCase::QUICK);
  ...
  AddTestCase (new WifiOfdmMaskSlopesTestCase ("11ac 160MHz", WIFI_PHY_STANDARD_80211ac,
                                               WIFI_PHY_BAND_5GHZ, 160, maskSlopesLeft,
                                               maskSlopesRight, tol),
               TestCase::QUICK);
  ...
  AddTestCase (new WifiOfdmMaskSlopesTestCase ("11ax_2.4GHz 20MHz", WIFI_PHY_STANDARD_80211ax,
                                               WIFI_PHY_BAND_2_4GHZ, 20, maskSlopesLeft,
                                               maskSlopesRight, tol),
               TestCase::QUICK);
  ...
  maskSlopesRight.push_back (std::make_pair (641, -28.094)); // Outer band right (start)
  maskSlopesRight.push_back (std::make_pair (768, -40.0)); // Outer band right (stop)
  AddTestCase (new WifiOfdmMaskSlopesTestCase ("11ax_5GHz 20MHz", WIFI_PHY_STANDARD_80211ax,
                                               WIFI_PHY_BAND_5GHZ, 20, maskSlopesLeft,
                                               maskSlopesRight, tol),
               TestCase::QUICK);
  ...
  AddTestCase (new WifiOfdmMaskSlopesTestCase ("11ax_2.4GHz 40MHz", WIFI_PHY_STANDARD_80211ax,
                                               WIFI_PHY_BAND_2_4GHZ, 40, maskSlopesLeft,
                                               maskSlopesRight, tol),
               TestCase::QUICK);
  ...
  maskSlopesRight.push_back (std::make_pair (1281, -28.047)); // Outer band right (start)
  maskSlopesRight.push_back (std::make_pair (1536, -40.0)); // Outer band right (stop)
  AddTestCase (new WifiOfdmMaskSlopesTestCase ("11ax_5GHz 40MHz", WIFI_PHY_STANDARD_80211ax,
                                               WIFI_PHY_BAND_5GHZ, 40, maskSlopesLeft,
                                               maskSlopesRight, tol),
               TestCase::QUICK);
  ...
  AddTestCase (new WifiOfdmMaskSlopesTestCase ("11ax_2.4GHz 80MHz", WIFI_PHY_STANDARD_80211ax,
                                               WIFI_PHY_BAND_2_4GHZ, 80, maskSlopesLeft,
                                               maskSlopesRight, tol),
               TestCase::QUICK);
  ...
  AddTestCase (new WifiOfdmMaskSlopesTestCase ("11ax_5GHz 80MHz", WIFI_PHY_STANDARD_80211ax,
                                               WIFI_PHY_BAND_5GHZ, 80, maskSlopesLeft,
                                               maskSlopesRight, tol),
               TestCase::QUICK);
  ...
  AddTestCase (new WifiOfdmMaskSlopesTestCase ("11ax_5GHz 160MHz", WIFI_PHY_STANDARD_80211ax,
                                               WIFI_PHY_BAND_5GHZ, 160, maskSlopesLeft,
                                               maskSlopesRight, tol),
               TestCase::QUICK);
