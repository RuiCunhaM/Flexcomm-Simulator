...
XmlConfigSave::XmlConfigSave () : m_writer (0)
{
...
XmlConfigSave::Default (void)
{
  ...
  private:
    virtual void
    StartVisitTypeId (std::string name)
    {
      m_typeid = name;
    }
    virtual void
    DoVisitAttribute (std::string name, std::string defaultValue)
    {
      ...
      rc = xmlTextWriterWriteAttribute (m_writer, BAD_CAST "value", BAD_CAST defaultValue.c_str ());
      if (rc < 0)
        {
          NS_FATAL_ERROR ("Error at xmlTextWriterWriteAttribute");
        }
...
XmlConfigSave::Attributes (void)
{
  ...
  private:
    ...
    DoVisitAttribute (Ptr<Object> object, std::string name)
    {
      ...
      rc = xmlTextWriterWriteAttribute (m_writer, BAD_CAST "value", BAD_CAST str.Get ().c_str ());
      if (rc < 0)
        {
          NS_FATAL_ERROR ("Error at xmlTextWriterWriteAttribute");
        }
