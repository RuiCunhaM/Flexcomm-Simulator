...
EpcMmeApplication::EpcMmeApplication () : m_gtpcUdpPort (2123) //fixed by the standard
{
    ...
    std::list<EpcS1apSapMme::ErabSwitchedInDownlinkItem> erabToBeSwitchedInDownlinkList)
{
  ...
  for (auto &erab : erabToBeSwitchedInDownlinkList)
    {
      NS_LOG_DEBUG ("erabId " << erab.erabId << " eNB " << erab.enbTransportLayerAddress << " TEID "
                              << erab.enbTeid);
...
EpcMmeApplication::DoRecvModifyBearerResponse (GtpcHeader &header, Ptr<Packet> packet)
{
  ...
  jt->second->s1apSapEnb->PathSwitchRequestAcknowledge (enbUeS1Id, mmeUeS1Id, cellId,
