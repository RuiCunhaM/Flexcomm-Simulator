...
LteEnbRrc::GetTypeId (void)
{
  ...
  static TypeId tid =
      TypeId ("ns3::LteEnbRrc")
          ...
          .AddAttribute (
              "SrsPeriodicity", "The SRS periodicity in milliseconds", UintegerValue (40),
          ...
          .AddTraceSource ("NotifyConnectionRelease", "trace fired when an UE is released",
                           MakeTraceSourceAccessor (&LteEnbRrc::m_connectionReleaseTrace),
                           "ns3::LteEnbRrc::ConnectionHandoverTracedCallback")
          .AddTraceSource ("RrcTimeout", "trace fired when a timer expires",
...
LteEnbRrc::RemoveUe (uint16_t rnti)
{
  ...
  m_connectionReleaseTrace (imsi, ComponentCarrierToCellId (it->second->GetComponentCarrierId ()),
                            rnti);
  m_ueMap.erase (it);
  for (uint8_t i = 0; i < m_numberOfComponentCarriers; i++)
    {
