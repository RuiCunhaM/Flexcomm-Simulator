...
NoOpComponentCarrierManager::DoRemoveUe (uint16_t rnti)
{
  ...
  NS_ASSERT_MSG (rlcLcIt != m_rlcLcInstantiated.end (),
                 "request to Release Data Radio Bearer on UE without Logical Channels enabled");

  auto rntiIt = m_ueAttached.find (rnti);

  NS_ASSERT_MSG (rntiIt != m_ueAttached.end (),
                 "request to Release Data Radio Bearer on unattached UE");
...
NoOpComponentCarrierManager::DoSetupDataRadioBearer (EpsBearer bearer, uint8_t bearerId,
