...
public:
   ...
   * \param [in] count
   */
  typedef void (*PhySyncDetectionTracedCallback) (uint64_t imsi, uint16_t rnti, uint16_t cellId,
                                                  std::string type, uint16_t count);
   ...
   * \param [in] count
   */
  typedef void (*ImsiCidRntiCountTracedCallback) (uint64_t imsi, uint16_t cellId, uint16_t rnti,
                                                  uint8_t count);
...
private:
  ...
  uint8_t
      m_noOfSyncIndications; ///< number of in-sync or out-of-sync indications coming from PHY layer

  bool
      m_leaveConnectedMode; ///< true if UE NAS ask UE RRC to leave connected mode, e.g., after RLF, i.e. T310 has expired

  uint16_t m_previousCellId; ///< the cell id of the previous cell UE was attached to

  uint8_t
      m_connEstFailCountLimit; ///< the counter value for T300 timer expiration received from the eNB
