...
EpsBearer::EpsBearer (Qci x, struct GbrQosInformation y) : ObjectBase (), qci (x), gbrQosInfo (y)
{
  ObjectBase::ConstructSelf (AttributeConstructionList ());
}
  ...
  if (ret.size () == 0)
    {
      ...
      ret.insert (
          std::make_pair (GBR_NON_CONV_VIDEO, std::make_tuple (true, 5, 300, 1.0e-6, 0, 0)));
...
EpsBearer::GetRequirementsRel15 ()
{
  ...
  if (ret.size () == 0)
    {
      ret.insert (
          std::make_pair (GBR_CONV_VOICE, std::make_tuple (true, 20, 100, 1.0e-2, 0, 2000)));
      ret.insert (
          std::make_pair (GBR_CONV_VIDEO, std::make_tuple (true, 40, 150, 1.0e-3, 0, 2000)));
      ...
      ret.insert (
          std::make_pair (NGBR_LOW_LAT_EMBB, std::make_tuple (false, 68, 10, 1.0e-6, 0, 0)));
