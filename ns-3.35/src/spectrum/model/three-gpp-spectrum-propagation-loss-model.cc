...
ThreeGppSpectrumPropagationLossModel::CalcBeamformingGain (
    ...
    Ptr<const MatrixBasedChannelModel::ChannelMatrix> params, const ns3::Vector &sSpeed,
  ...
  for (uint8_t cIndex = 0; cIndex < numCluster; cIndex++)
    {
      ...
      double temp_doppler =
          2 * M_PI *
          ((sin (params->m_angle[MatrixBasedChannelModel::ZOA_INDEX][cIndex] * M_PI / 180) *
                cos (params->m_angle[MatrixBasedChannelModel::AOA_INDEX][cIndex] * M_PI / 180) *
                uSpeed.x +
            sin (params->m_angle[MatrixBasedChannelModel::ZOA_INDEX][cIndex] * M_PI / 180) *
                sin (params->m_angle[MatrixBasedChannelModel::AOA_INDEX][cIndex] * M_PI / 180) *
                uSpeed.y +
            cos (params->m_angle[MatrixBasedChannelModel::ZOA_INDEX][cIndex] * M_PI / 180) *
                uSpeed.z) +
           (sin (params->m_angle[MatrixBasedChannelModel::ZOD_INDEX][cIndex] * M_PI / 180) *
                cos (params->m_angle[MatrixBasedChannelModel::AOD_INDEX][cIndex] * M_PI / 180) *
                sSpeed.x +
            sin (params->m_angle[MatrixBasedChannelModel::ZOD_INDEX][cIndex] * M_PI / 180) *
                sin (params->m_angle[MatrixBasedChannelModel::AOD_INDEX][cIndex] * M_PI / 180) *
                sSpeed.y +
            cos (params->m_angle[MatrixBasedChannelModel::ZOD_INDEX][cIndex] * M_PI / 180) *
                sSpeed.z) +
           2 * alpha * D) *
          ...
          for (uint8_t cIndex = 0; cIndex < numCluster; cIndex++)
            {
              ...
              subsbandGain = subsbandGain + longTerm[cIndex] * doppler[cIndex] *
                                                exp (std::complex<double> (0, delay));
            }
...
ThreeGppSpectrumPropagationLossModel::GetLongTerm (
    uint32_t aId, uint32_t bId, Ptr<const MatrixBasedChannelModel::ChannelMatrix> channelMatrix,
    const PhasedArrayModel::ComplexVector &aW, const PhasedArrayModel::ComplexVector &bW) const
{
  PhasedArrayModel::ComplexVector
...
ThreeGppSpectrumPropagationLossModel::DoCalcRxPowerSpectralDensity (
    Ptr<const SpectrumValue> txPsd, Ptr<const MobilityModel> a, Ptr<const MobilityModel> b) const
{
  NS_LOG_FUNCTION (this);
  ...
  NS_ASSERT_MSG (m_deviceAntennaMap.find (aId) != m_deviceAntennaMap.end (),
  ...
  NS_ASSERT_MSG (m_deviceAntennaMap.find (bId) != m_deviceAntennaMap.end (),
  ...
  Ptr<const MatrixBasedChannelModel::ChannelMatrix> channelMatrix =
      m_channelModel->GetChannel (a, b, aAntenna, bAntenna);
