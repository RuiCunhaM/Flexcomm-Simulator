...
FlameRtable::GetTypeId ()
{
  static TypeId tid =
      TypeId ("ns3::flame::FlameRtable")
          ...
          .AddAttribute ("Lifetime", "The lifetime of the routing entry", TimeValue (Seconds (120)),
                         MakeTimeAccessor (&FlameRtable::m_lifetime), MakeTimeChecker ());
  return tid;
}
...
FlameRtable::Lookup (Mac48Address destination)
{
  ...
  return LookupResult (i->second.retransmitter, i->second.interface, i->second.cost,
                       i->second.seqnum);
}
bool
