...
public:
  ...
  struct LookupResult
  {
    ...
    LookupResult (Mac48Address r = Mac48Address::GetBroadcast (), uint32_t i = INTERFACE_ANY,
                  uint32_t m = MAX_METRIC, uint32_t s = 0, Time l = Seconds (0.0));
    /**
...
public:
   ...
   */
  void AddReactivePath (Mac48Address destination, Mac48Address retransmitter, uint32_t interface,
   ...
   * \param seqnum  the sequence number
   */
  void AddProactivePath (uint32_t metric, Mac48Address root, Mac48Address retransmitter,
