...
private:
  ...
  bool
      m_isFullCwndSent; //!< True if all bytes for that cWnd is sent and sender is waiting for an ACK
  ...
  Time
      m_expectedInterval; //!< Theoretical estimate of the time at which next packet is scheduled for transmission
  uint32_t m_packetsSent; //!< Number of packets sent by sender so far
  Time
      m_nextPacketInterval; //!< Time maintained by Tx () trace about interval at which next packet will be sent
  Time
      m_tracedRtt; //!< Traced value of RTT, which may be different from the environment RTT in case of delayed ACKs
};
