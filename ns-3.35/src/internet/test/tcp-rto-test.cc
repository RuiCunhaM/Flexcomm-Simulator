...
protected:
  ...
  virtual void ProcessedAck (const Ptr<const TcpSocketState> tcb, const TcpHeader &h,
                             SocketWho who);
...
TcpSsThreshRtoTest::TcpSsThreshRtoTest (TypeId &congControl, uint32_t seqToDrop, Time minRto,
...
public:
  TcpRtoTestSuite () : TestSuite ("tcp-rto-test", UNIT)
  {
    ...
    for (std::list<TypeId>::iterator it = types.begin (); it != types.end (); ++it)
      {
        AddTestCase (new TcpRtoTest ((*it), (*it).GetName () + " RTO retransmit testing"),
                     TestCase::QUICK);
        uint32_t seqToDrop = 25001;
        Time minRto = Seconds (0.5);
        // With RTO of 0.5 seconds, BytesInFlight winds down to zero before RTO
        AddTestCase (
            new TcpSsThreshRtoTest ((*it), seqToDrop, minRto,
                                    (*it).GetName () + " RTO ssthresh testing, set to 2*MSL"),
            TestCase::QUICK);
        // With RTO of 0.005 seconds, FlightSize/2 > 2*SMSS
        minRto = Seconds (0.005);
        AddTestCase (new TcpSsThreshRtoTest (
                         (*it), seqToDrop, minRto,
                         (*it).GetName () + " RTO ssthresh testing, set to half of BytesInFlight"),
                     TestCase::QUICK);
        AddTestCase (new TcpTimeRtoTest ((*it), (*it).GetName () + " RTO timing testing"),
                     TestCase::QUICK);
      }
  }
};
