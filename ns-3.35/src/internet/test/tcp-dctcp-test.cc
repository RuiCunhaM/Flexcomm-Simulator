      ...
      if (m_senderReceived == 2 && m_testCase == 3)
        {
          NS_TEST_ASSERT_MSG_NE (
              ...
              "The flag ECE should be set in TCP header of the packet sent by the receiver when it "
              "receives a packet with CE bit set in IP header");
        }
      if (m_senderReceived > 2 && m_testCase == 3)
        {
...
public:
  TcpDctcpTestSuite () : TestSuite ("tcp-dctcp-test", UNIT)
  {
    ...
    AddTestCase (new TcpDctcpCodePointsTest (
                 ...
                 TestCase::QUICK);
    AddTestCase (new TcpDctcpCodePointsTest (3, "ECE Functionality Test: ECE should only be sent "
                                                "by receiver when it receives CE flags"),
                 TestCase::QUICK);
  }
};
