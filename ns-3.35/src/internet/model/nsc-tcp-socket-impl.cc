...
#define NS_LOG_APPEND_CONTEXT                                                               \
  if (m_node)                                                                               \
    {                                                                                       \
      std::clog << Simulator::Now ().As (Time::S) << " [node " << m_node->GetId () << "] "; \
    }
...
NscTcpSocketImpl::FinishBind (void)
{
  ...
  m_endPoint->SetRxCallback (
      MakeCallback (&NscTcpSocketImpl::ForwardUp, Ptr<NscTcpSocketImpl> (this)));
  m_endPoint->SetDestroyCallback (
      MakeCallback (&NscTcpSocketImpl::Destroy, Ptr<NscTcpSocketImpl> (this)));
...
Ptr<Packet>
NscTcpSocketImpl::RecvFrom (uint32_t maxSize, uint32_t flags, Address &fromAddress)
{
  NS_LOG_FUNCTION (this << maxSize << flags);
...
void
NscTcpSocketImpl::CompleteFork (void)
{
  ...
  NS_LOG_LOGIC ("NscTcpSocketImpl " << this << " accepted connection from " << m_remoteAddress
                                    << ":" << m_remotePort << " to " << m_localAddress << ":"
                                    << m_localPort);
...
void
NscTcpSocketImpl::ConnectionSucceeded ()
{ // We would preferred to have scheduled an event directly to
  ...
  NS_LOG_LOGIC ("NscTcpSocketImpl " << this << " connected to " << m_remoteAddress << ":"
                                    << m_remotePort << " from " << m_localAddress << ":"
                                    << m_localPort);
  NotifyConnectionSucceeded ();
}

bool
NscTcpSocketImpl::Accept (void)
{
  if (m_state == CLOSED)
...
bool
NscTcpSocketImpl::ReadPendingData (void)
{
  if (m_state != ESTABLISHED)
    {
      return false;
...
bool
NscTcpSocketImpl::SendPendingData (void)
{
  NS_LOG_FUNCTION (this);
...
Ptr<NscTcpSocketImpl>
NscTcpSocketImpl::Copy ()
{
...
NscTcpSocketImpl::GetNativeNs3Errno (int error) const
{
  ...
  switch (err)
    ...
    case NSC_ETIMEDOUT:
      return ERROR_NOTCONN; /// \todo this mapping isn't correct
    case NSC_ENOTDIR: // used by eg. sysctl(2). Shouldn't happen normally,
    // but is triggered by e.g. show_config().
    case NSC_EUNKNOWN:
      return ERROR_INVAL; // Catches stacks that 'return -1' without real mapping
    }
