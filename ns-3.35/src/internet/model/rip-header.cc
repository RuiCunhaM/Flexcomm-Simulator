...
TypeId
RipRte::GetInstanceTypeId (void) const
{
  return GetTypeId ();
}
...
TypeId
RipHeader::GetInstanceTypeId (void) const
{
  return GetTypeId ();
}
...
uint16_t
RipHeader::GetRteNumber (void) const
{
  return m_rteList.size ();
}

std::list<RipRte>
RipHeader::GetRteList (void) const
{
  return m_rteList;
}
