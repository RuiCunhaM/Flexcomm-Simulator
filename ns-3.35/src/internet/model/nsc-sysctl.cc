...
bool
NscStackStringAccessor::HasGetter (void) const
{
  return true;
}

bool
NscStackStringAccessor::HasSetter (void) const
{
  return true;
}

bool
NscStackStringAccessor::Set (ObjectBase *object, const AttributeValue &val) const
{
...
bool
NscStackStringAccessor::Get (const ObjectBase *object, AttributeValue &val) const
{
      ...
      for (int i = 0; m_stack->sysctl_getnum (i, buf, sizeof (buf)) > 0; i++)
        {
          ...
          if (m_stack->sysctl_get (buf, value, sizeof (value)) > 0)
            {
              tid.AddAttribute (buf, "Help text", StringValue (value),
                                Create<NscStackStringAccessor> (buf), MakeStringChecker ());
            }
        }
      return tid;
    }
...
{
  static TypeId tid = TypeId ("ns3::Ns3NscStack").SetParent<Object> ();
  return tid;
}
