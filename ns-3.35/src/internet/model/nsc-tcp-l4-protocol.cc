...
NscInterfaceImpl::NscInterfaceImpl (Ptr<NscTcpL4Protocol> prot) : m_prot (prot)
{
}
...
NscTcpL4Protocol::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::NscTcpL4Protocol")
          ...
          .AddAttribute ("Library", "Set the linux library to be used to create the stack",
                         TypeId::ATTR_GET | TypeId::ATTR_CONSTRUCT,
                         StringValue ("liblinux2.6.26.so"),
                         MakeStringAccessor (&NscTcpL4Protocol::GetNscLibrary,
                                             &NscTcpL4Protocol::SetNscLibrary),
                         MakeStringChecker ());
  return tid;
}
...
NscTcpL4Protocol::DoDispose (void)
{
  ...
  for (std::vector<Ptr<NscTcpSocketImpl>>::iterator i = m_sockets.begin (); i != m_sockets.end ();
       i++)
    {
      *i = 0;
    }
...
Ipv4EndPoint *
NscTcpL4Protocol::Allocate (Ptr<NetDevice> boundNetDevice, Ipv4Address localAddress,
...
NscTcpL4Protocol::Receive (Ptr<Packet> packet, Ipv4Header const &header,
...
void
NscTcpL4Protocol::SoftInterrupt (void)
{
...
void
NscTcpL4Protocol::send_callback (const void *data, int datalen)
{
  Ptr<Packet> p;
...
void
NscTcpL4Protocol::gettime (unsigned int *sec, unsigned int *usec)
{
...
void
NscTcpL4Protocol::AddInterface (void)
{
  ...
  for (uint32_t i = 1; i < nInterfaces; i++)
    {
      ...
      NS_LOG_LOGIC ("if_attach " << addrOss.str ().c_str () << " " << maskOss.str ().c_str () << " "
                                 << mtu);
