...
TypeId
RipNgRte::GetInstanceTypeId (void) const
{
  return GetTypeId ();
}
...
TypeId
RipNgHeader::GetInstanceTypeId (void) const
{
  return GetTypeId ();
}
...
uint16_t
RipNgHeader::GetRteNumber (void) const
{
  return m_rteList.size ();
}

std::list<RipNgRte>
RipNgHeader::GetRteList (void) const
{
  return m_rteList;
}
