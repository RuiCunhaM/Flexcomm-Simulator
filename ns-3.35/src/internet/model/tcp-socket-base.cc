...
TcpSocketBase::DoConnect (void)
{
  ...
  if (m_state == CLOSED || m_state == LISTEN || m_state == SYN_SENT || m_state == LAST_ACK ||
    ...
    { // send a SYN packet and change state into SYN_SENT
      ...
      m_state = SYN_SENT;
      m_tcb->m_ecnState = TcpSocketState::
          ECN_DISABLED; // because sender is not yet aware about receiver's ECN capability
    }
          ...
          if (m_timestampEnabled && tcpHeader.HasOption (TcpOption::TS))
            {
              ...
              if (m.IsZero ())
                {
                  NS_LOG_LOGIC ("TcpSocketBase::EstimateRtt - RTT calculated from TcpOption::TS is "
                                "zero, approximating to 1us.");
                  m = MicroSeconds (1);
                }
            }
          else
