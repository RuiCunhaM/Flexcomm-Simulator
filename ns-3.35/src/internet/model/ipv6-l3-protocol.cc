  ...
  if (nextHeader == Ipv6Header::IPV6_EXT_HOP_BY_HOP)
    {
      ...
      if (ipv6Extension)
        {
          ipv6Extension->Process (packet, 0, hdr, hdr.GetDestination (), (uint8_t *) 0,
...
void
Ipv6L3Protocol::SendRealOut (Ptr<Ipv6Route> route, Ptr<Packet> packet, Ipv6Header const &ipHeader)
{
  ...
  if (route->GetGateway () != Ipv6Address::GetAny ())
    {
      if (outInterface->IsUp ())
      ...
      else
        {
          ...
          m_dropTrace (ipHeader, packet, DROP_INTERFACE_DOWN, m_node->GetObject<Ipv6> (),
                       interface);
        }
    }
  else
...
void
Ipv6L3Protocol::LocalDeliver (Ptr<const Packet> packet, Ipv6Header const &ip, uint32_t iif)
{
