  ...
  static TypeId tid =
      TypeId ("ns3::ProbabilisticV2vUrbanChannelConditionModel")
          ...
          .AddAttribute (
              ...
              MakeEnumChecker (VehicleDensity::LOW, "Low", VehicleDensity::MEDIUM, "Medium",
                                                          ...
                                                          Ptr<const MobilityModel> b) const
{
  ...
  switch (m_densityUrban)
    {
    case VehicleDensity::LOW:
      pNlosv = std::min (1.0, std::max (0.0, 1 / (0.0396 * distance2D) *
                                                 exp (-(log (distance2D) - 5.2718) *
                                                      (log (distance2D) - 5.2718) / 3.4827)));
      break;
    case VehicleDensity::MEDIUM:
      pNlosv = std::min (1.0, std::max (0.0, 1 / (0.0312 * distance2D) *
                                                 exp (-(log (distance2D) - 5.0063) *
                                                      (log (distance2D) - 5.0063) / 2.4544)));
      break;
    case VehicleDensity::HIGH:
      pNlosv = std::min (1.0, std::max (0.0, 1 / (0.0242 * distance2D) *
