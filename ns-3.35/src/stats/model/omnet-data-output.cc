  ...
  for (MetadataList::iterator i = dc.MetadataBegin (); i != dc.MetadataEnd (); i++)
    {
      ...
      if (isNumeric (blob.second))
        {
          scalarFile << "scalar . \"" << blob.first << "\" \"" << blob.second << "\"" << std::endl;
        }
    }
...
OmnetDataOutput::OmnetOutputCallback::OmnetOutputCallback (std::ostream *scalar) : m_scalar (scalar)
{
  NS_LOG_FUNCTION (this << scalar);
}

void
OmnetDataOutput::OmnetOutputCallback::OutputStatistic (std::string context, std::string name,
...
void
OmnetDataOutput::OmnetOutputCallback::OutputSingleton (std::string context, std::string name,
                                                       int val)
{
