  ...
  for (uint32_t i = 1; i <= numPackets; i++)
    {
      ...
      NS_TEST_EXPECT_MSG_EQ (q->GetCurrentSize ().GetValue (), i * modeSize,
                             "There should be " << i << " packet(s) in there");
    }

  // no room for another packet
  NS_TEST_EXPECT_MSG_EQ (q->Enqueue (Create<FifoQueueDiscTestItem> (p, dest)), false,
  ...
  for (uint32_t i = 1; i <= numPackets; i++)
    {
      ...
      NS_TEST_EXPECT_MSG_EQ (q->GetCurrentSize ().GetValue (), (numPackets - i) * modeSize,
                             "There should be " << numPackets - i << " packet(s) in there");
      NS_TEST_EXPECT_MSG_EQ (item->GetPacket ()->GetUid (), uids[i - 1],
...
FifoQueueDiscTestCase::RunFifoTest (QueueSizeUnit mode)
{
  ...
  NS_TEST_EXPECT_MSG_EQ (queue->GetNInternalQueues (), 0,
                         "Verify that the queue disc has no internal queue");

  NS_TEST_EXPECT_MSG_EQ (queue->SetAttributeFailSafe (
  ...
  NS_TEST_EXPECT_MSG_EQ (queue->GetNInternalQueues (), 1,
