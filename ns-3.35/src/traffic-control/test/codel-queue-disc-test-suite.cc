...
CoDelQueueDiscBasicEnqueueDequeue::DoRun (void)
{
  ...
  NS_TEST_EXPECT_MSG_EQ (queue->GetCurrentSize ().GetValue (), 5 * modeSize,
                         "There should be five packets in queue");
  queue->Enqueue (Create<CodelQueueDiscTestItem> (p6, dest, false));
  NS_TEST_EXPECT_MSG_EQ (queue->GetCurrentSize ().GetValue (), 6 * modeSize,
                         "There should be six packets in queue");

  NS_TEST_EXPECT_MSG_EQ (queue->GetStats ().GetNDroppedPackets (CoDelQueueDisc::OVERLIMIT_DROP), 0,
  ...
  NS_TEST_EXPECT_MSG_EQ (queue->GetCurrentSize ().GetValue (), 5 * modeSize,
                         "There should be five packets in queue");
  NS_TEST_EXPECT_MSG_EQ (item->GetPacket ()->GetUid (), p1->GetUid (),
  ...
  NS_TEST_EXPECT_MSG_EQ (queue->GetCurrentSize ().GetValue (), 4 * modeSize,
                         "There should be four packets in queue");
  NS_TEST_EXPECT_MSG_EQ (item->GetPacket ()->GetUid (), p2->GetUid (),
  ...
  NS_TEST_EXPECT_MSG_EQ (queue->GetCurrentSize ().GetValue (), 3 * modeSize,
                         "There should be three packets in queue");
  NS_TEST_EXPECT_MSG_EQ (item->GetPacket ()->GetUid (), p3->GetUid (),
  ...
  NS_TEST_EXPECT_MSG_EQ (queue->GetCurrentSize ().GetValue (), 2 * modeSize,
                         "There should be two packets in queue");
  NS_TEST_EXPECT_MSG_EQ (item->GetPacket ()->GetUid (), p4->GetUid (),
  ...
  NS_TEST_EXPECT_MSG_EQ (queue->GetCurrentSize ().GetValue (), 1 * modeSize,
                         "There should be one packet in queue");
  NS_TEST_EXPECT_MSG_EQ (item->GetPacket ()->GetUid (), p5->GetUid (),
  ...
  NS_TEST_EXPECT_MSG_EQ (item->GetPacket ()->GetUid (), p6->GetUid (),
...
CoDelQueueDiscBasicOverflow::DoRun (void)
{
  ...
  NS_TEST_EXPECT_MSG_EQ (queue->GetCurrentSize ().GetValue (), 500 * modeSize,
                         "There should be 500 packets in queue");
  NS_TEST_EXPECT_MSG_EQ (queue->GetStats ().GetNDroppedPackets (CoDelQueueDisc::OVERLIMIT_DROP), 3,
      ...
      for (uint16_t recInvSqrt = 0xff; recInvSqrt > 0; recInvSqrt /= 2)
        {
          ...
          NS_TEST_EXPECT_MSG_EQ (ns3Result, linuxResult,
...
CoDelQueueDiscBasicDrop::DoRun (void)
{
  ...
  NS_TEST_EXPECT_MSG_EQ (queue->GetCurrentSize ().GetValue (), 20 * modeSize,
          ...
          if (initialTargetMarkCount == 0 && currentTime > queue->GetTarget ())
            {
              if (currentTime < queue->GetInterval ())
              ...
              else if (currentTime >= queue->GetInterval ())
                {
                  ...
                  NS_TEST_EXPECT_MSG_EQ (currentDropCount, 0,
                                         "There should not be any packet drops");
                  NS_TEST_EXPECT_MSG_EQ (currentTargetMarkCount, 1,
            ...
            { // In dropping state
              if (currentTime.GetMicroSeconds () < initialDropNext)
              ...
              else if (currentTime.GetMicroSeconds () >= initialDropNext)
                {
                  ...
                  NS_TEST_EXPECT_MSG_EQ (currentDropCount, 0,
                                         "There should not be any packet drops");
                  NS_TEST_EXPECT_MSG_EQ (currentTargetMarkCount, 2,
          ...
          if (initialTargetMarkCount == 0 && currentTime > queue->GetTarget ())
            {
              if (currentTime < queue->GetInterval ())
              ...
              else if (currentTime >= queue->GetInterval ())
                {
                  ...
                  NS_TEST_EXPECT_MSG_EQ (currentTargetMarkCount, 1,
                                         "There should be 1 target exceeded marked packet");
                  NS_TEST_EXPECT_MSG_EQ (currentCeThreshMarkCount, 1,
              ...
              if (currentTime.GetMicroSeconds () < initialDropNext)
                {
                  ...
                  NS_TEST_EXPECT_MSG_EQ (currentTargetMarkCount, 1,
                                         "There should still be only 1 target exceeded marked "
                                         "packet from the last dequeue");
                  NS_TEST_EXPECT_MSG_EQ (currentCeThreshMarkCount, 2,
          ...
          else if (initialCeThreshMarkCount > 0 && currentTime < queue->GetInterval ())
            {
              if (initialCeThreshMarkCount < 2)
                ...
                { // In dropping state
                  ...
                  NS_TEST_EXPECT_MSG_EQ (currentDropCount, 0,
                                         "There should not be any packet drops");
                  NS_TEST_EXPECT_MSG_EQ (currentCeThreshMarkCount, 3,
