...
PieQueueDiscTestCase::RunPieTest (QueueSizeUnit mode)
{
  ...
  NS_TEST_EXPECT_MSG_EQ (queue->GetCurrentSize ().GetValue (), 8 * modeSize,
  ...
  NS_TEST_EXPECT_MSG_EQ (queue->GetCurrentSize ().GetValue (), 7 * modeSize,
  ...
  NS_TEST_EXPECT_MSG_EQ (queue->SetAttributeFailSafe ("DequeueThreshold", UintegerValue (10000)),
  ...
  NS_TEST_EXPECT_MSG_EQ (st.GetNDroppedPackets (PieQueueDisc::FORCED_DROP), 0,
  ...
  NS_TEST_EXPECT_MSG_EQ (
      queue->SetAttributeFailSafe ("MaxSize", QueueSizeValue (QueueSize (mode, qSize))), true,
      "Verify that we can actually set the attribute MaxSize");
  NS_TEST_EXPECT_MSG_EQ (queue->SetAttributeFailSafe ("UseCapDropAdjustment", BooleanValue (false)),
                         true,
  ...
  NS_TEST_EXPECT_MSG_EQ (
      queue->SetAttributeFailSafe ("MaxSize", QueueSizeValue (QueueSize (mode, qSize))), true,
      "Verify that we can actually set the attribute MaxSize");
  NS_TEST_EXPECT_MSG_EQ (queue->SetAttributeFailSafe ("UseCapDropAdjustment", BooleanValue (true)),
                         true,
                         "Verify that we can actually set the attribute UseCapDropAdjustment");
  queue->Initialize ();
  testAttributes->m_checkProb = true;
  ...
  NS_TEST_EXPECT_MSG_NE (st.GetNDroppedPackets (PieQueueDisc::UNFORCED_DROP), 0,
  ...
  NS_TEST_EXPECT_MSG_GT (testAttributes->m_maxDropProb, 0.3,
  ...
  NS_TEST_EXPECT_MSG_EQ (
      queue->SetAttributeFailSafe ("MaxSize", QueueSizeValue (QueueSize (mode, qSize))), true,
      "Verify that we can actually set the attribute MaxSize");
  NS_TEST_EXPECT_MSG_EQ (queue->SetAttributeFailSafe ("UseDerandomization", BooleanValue (true)),
  ...
  NS_TEST_EXPECT_MSG_EQ (st.GetNDroppedPackets (PieQueueDisc::FORCED_DROP), 0,
                         "There should be zero forced drops");
  NS_TEST_EXPECT_MSG_EQ (testAttributes->m_accuProbError, 0.0,
  ...
  NS_TEST_EXPECT_MSG_EQ (st.GetNMarkedPackets (PieQueueDisc::UNFORCED_MARK), 0,
