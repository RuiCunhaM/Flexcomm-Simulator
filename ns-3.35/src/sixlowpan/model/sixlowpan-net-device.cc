                                       ...
                                       PacketType packetType)
{
      ...
      if (!hasBc0)
        {
          ...
          m_dropTrace (DROP_UNKNOWN_EXTENSION, copyPkt, m_node->GetObject<SixLowPanNetDevice> (),
                       GetIfIndex ());
          return;
        }
  ...
  switch (dispatchVal)
    ...
    case SixLowPanDispatch::LOWPAN_HC1:
      if (m_useIphc)
        {
          m_dropTrace (DROP_DISALLOWED_COMPRESSION, copyPkt,
    ...
    case SixLowPanDispatch::LOWPAN_IPHC:
      ...
      if (DecompressLowPanIphc (copyPkt, realSrc, realDst))
        {
          m_dropTrace (DROP_SATETFUL_DECOMPRESSION_PROBLEM, copyPkt,
                       m_node->GetObject<SixLowPanNetDevice> (), GetIfIndex ());
        }
      else
        {
...
SixLowPanNetDevice::GetChannel (void) const
{
  ...
  NS_ASSERT_MSG (m_netDevice != 0,
                 "Sixlowpan: can't find any lower-layer protocol " << m_netDevice);
  ...
  switch (actualEncodedHeaderType)
    ...
    case SixLowPanNhcExtension::EID_IPv6_H:
      ...
      if (DecompressLowPanIphc (packet, src, dst))
        {
          m_dropTrace (DROP_SATETFUL_DECOMPRESSION_PROBLEM, packet,
                       m_node->GetObject<SixLowPanNetDevice> (), GetIfIndex ());
      ...
      switch (dispatchValFrag1)
        ...
        case SixLowPanDispatch::LOWPAN_IPHC:
          if (DecompressLowPanIphc (p, src, dst))
            {
              m_dropTrace (DROP_SATETFUL_DECOMPRESSION_PROBLEM, p,
                           m_node->GetObject<SixLowPanNetDevice> (), GetIfIndex ());
              return false;
            }
          break;
      ...
      if (m_fragmentReassemblyListSize && (m_fragments.size () >= m_fragmentReassemblyListSize))
        {
          ...
          for (std::list<Ptr<Packet>>::iterator fragIter = storedFragments.begin ();
            ...
            {
              m_dropTrace (DROP_FRAGMENT_BUFFER_FULL, *fragIter,
                           m_node->GetObject<SixLowPanNetDevice> (), GetIfIndex ());
            }
