  ...
  for (uint32_t i = 0; i < GetSize (); ++i)
    {
      ...
      MPI_Irecv (g_pRxBuffers[i], MAX_MPI_MSG_SIZE, MPI_CHAR, MPI_ANY_SOURCE, 0, g_communicator,
                 &g_requests[i]);
    }
}

void
GrantedTimeWindowMpiInterface::SendPacket (Ptr<Packet> p, const Time &rxTime, uint32_t node,
                                           uint32_t dev)
{
  ...
  while (true)
    {
      ...
      MPI_Irecv (g_pRxBuffers[index], MAX_MPI_MSG_SIZE, MPI_CHAR, MPI_ANY_SOURCE, 0, g_communicator,
                 &g_requests[index]);
    }
}
