...
public:
   ...
   */
  std::set<uint32_t>
  GetInterfaceExclusions (void) const
  {
...
private:
   ...
   */
  void AddEntry (const Ipv4Address &dest, const Ipv4Address &next, uint32_t interface,
                 uint32_t distance);
  /**
   * \brief Adds a new entry into the routing table.
   *
...
private:
  ...
  bool
      m_linkTupleTimerFirstTime; //!< Flag to indicate if it is the first time the LinkTupleTimer fires.
  /**
   ...
   * \param netmask  The network mask.
   */
  void AssociationTupleTimerExpire (Ipv4Address gatewayAddr, Ipv4Address networkAddr,
                                    Ipv4Mask netmask);
   ...
   */
  void ForwardDefault (olsr::MessageHeader olsrMessage, DuplicateTuple *duplicated,
   ...
   */
  void ProcessHello (const olsr::MessageHeader &msg, const Ipv4Address &receiverIface,
   ...
   */
  void LinkSensing (const olsr::MessageHeader &msg, const olsr::MessageHeader::Hello &hello,
