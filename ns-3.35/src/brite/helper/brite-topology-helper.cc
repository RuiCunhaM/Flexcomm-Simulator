...
BriteTopologyHelper::BriteTopologyHelper (std::string confFile, std::string seedFile,
...
BriteTopologyHelper::BriteTopologyHelper (std::string confFile)
    : m_confFile (confFile), m_numAs (0), m_topology (NULL), m_numNodes (0), m_numEdges (0)
{
  NS_LOG_FUNCTION (this);
...
BriteTopologyHelper::ConstructTopology ()
{
  ...
  for (BriteTopologyHelper::BriteEdgeInfoList::iterator it = m_briteEdgeInfoList.begin ();
       it != m_briteEdgeInfoList.end (); ++it)
    {
      ...
      m_netDevices.push_back (new NetDeviceContainer (m_britePointToPointHelper.Install (
