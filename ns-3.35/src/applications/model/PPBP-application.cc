...
{
  static TypeId tid =
      TypeId ("ns3::PPBPApplication")
          .SetParent<Application> ()
          .AddConstructor<PPBPApplication> ()
          .AddAttribute ("BurstIntensity", "The data rate of each burst.",
                         DataRateValue (DataRate ("1Mb/s")),
                         MakeDataRateAccessor (&PPBPApplication::m_cbrRate), MakeDataRateChecker ())
          .AddAttribute ("PacketSize", "The size of packets sent in on state", UintegerValue (1470),
          ...
          .AddAttribute ("MeanBurstTimeLength", "Mean burst duration",
                         ...
                         MakePointerChecker<RandomVariableStream> ())
          .AddAttribute ("H", "Hurst parameter", DoubleValue (0.7),
                         MakeDoubleAccessor (&PPBPApplication::m_h), MakeDoubleChecker<double> ())
          .AddAttribute ("Remote", "The address of the destination", AddressValue (),
                         MakeAddressAccessor (&PPBPApplication::m_peer), MakeAddressChecker ())
          .AddAttribute ("Protocol", "The type of protocol to use.",
                         TypeIdValue (UdpSocketFactory::GetTypeId ()),
                         MakeTypeIdAccessor (&PPBPApplication::m_protocolTid), MakeTypeIdChecker ())
...
PPBPApplication::PPBP () // Poisson Pareto Burst
{
  ...
  m_PoissonArrival =
      Simulator::Schedule (t_poisson_arrival, &PPBPApplication::PoissonArrival, this);
  ...
  m_ParetoDeparture = Simulator::Schedule (t_poisson_arrival + Seconds (t_pareto),
                                           &PPBPApplication::ParetoDeparture, this);
  m_ppbp = Simulator::Schedule (t_poisson_arrival, &PPBPApplication::PPBP, this);
}

void
PPBPApplication::PoissonArrival ()
{
  NS_LOG_FUNCTION_NOARGS ();
  ++m_activebursts;
  if (m_offPeriod)
...
PPBPApplication::StopApplication () // Called at time specified by Stop
{
  ...
  if (m_socket != 0)
    ...
    NS_LOG_WARN ("PPBPApplication found null socket to close in StopApplication");
}
