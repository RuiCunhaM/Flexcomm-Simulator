...
{
  static TypeId tid =
      TypeId ("ns3::HttpClient")
          .SetParent<Application> ()
          .AddConstructor<HttpClient> ()
          .AddAttribute ("RemoteAddress", "The destination Address of the outbound packets",
                         AddressValue (), MakeAddressAccessor (&HttpClient::m_peerAddress),
                         MakeAddressChecker ())
          .AddAttribute ("RemotePort", "The destination port of the outbound packets",
                         UintegerValue (80), MakeUintegerAccessor (&HttpClient::m_peerPort),
...
HttpClient::SendRequest (Ptr<Socket> socket, string url)
{
  ...
  NS_LOG_INFO ("HttpClient (" << m_clientAddress << ") >> Sending request for " << url
                              << " to server (" << Ipv4Address::ConvertFrom (m_peerAddress)
                              << ").");
  socket->Send (packet);
}
      ...
      if (m_bytesReceived == m_contentLength)
        {
          NS_LOG_DEBUG ("HttpClient (" << m_clientAddress << ") >> " << m_contentType << ": "
                                       << m_bytesReceived << " bytes of " << m_contentLength
                                       << " received.");
          m_contentLength = 0;
          ...
          if (m_contentType == "main/object")
            {
              NS_LOG_INFO ("HttpClient (" << m_clientAddress << ") >> " << m_contentType
                                          << " successfully received. There are "
                                          << m_numOfInlineObjects << " inline objects to request.");
              NS_LOG_DEBUG ("HttpClient (" << m_clientAddress
                                           << ") >> Requesting inline/object 1...");
              m_inlineObjLoaded = 0;
          ...
          else
            {
              ...
              if (m_inlineObjLoaded < m_numOfInlineObjects)
              ...
              else
                {
                  ...
                  NS_LOG_DEBUG ("HttpClient (" << m_clientAddress << ") >> Reading time: "
                                               << readingTime << " seconds.");
                  Simulator::Schedule (Seconds (readingTime), &HttpClient::SendRequest, this,
                                       socket, "main/object");
                }
            }
        }
          ...
          if (m_contentType == "main/object")
          ...
          else
            {
              ...
              if (m_inlineObjLoaded < m_numOfInlineObjects)
                {
                  NS_LOG_DEBUG ("HttpClient (" << m_clientAddress << ") >> " << m_contentType << " "
                                               << m_inlineObjLoaded << " of "
                                               << m_numOfInlineObjects
                                               << " successfully received."
                                                  "\nHttpClient >> Requesting inline/object "
                                               << m_inlineObjLoaded + 1);
                  SendRequest (socket, "inline/object");
                }
              else
