...
{
  static TypeId tid =
      TypeId ("ns3::HttpServer")
          .SetParent<Application> ()
          .AddConstructor<HttpServer> ()
          .AddAttribute ("Port", "Port on which we listen for incoming packets.",
                         UintegerValue (80), MakeUintegerAccessor (&HttpServer::m_port),
  ...
  if (url == "main/object")
    {
      ...
      Ptr<ExponentialRandomVariable> numOfInlineObjStream =
      ...
      NS_LOG_INFO ("HttpServer >> Sending response to client. Main Object Size ("
                   << mainObjectSize << " bytes). NumOfInlineObjects (" << numOfInlineObj << ").");
      s->Send (p);
    }
  else
    {
