...
OFSwitch13StatsCalculator::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::OFSwitch13StatsCalculator")
          ...
          .AddAttribute ("EwmaAlpha", "The EWMA alpha parameter for averaging statistics.",
                         DoubleValue (0.2),
                         MakeDoubleAccessor (&OFSwitch13StatsCalculator::m_alpha),
                         MakeDoubleChecker<double> (0.0, 1.0))
          .AddAttribute ("DumpTimeout", "The interval to update and dump switch statistics.",
                         TimeValue (Seconds (1)),
                         MakeTimeAccessor (&OFSwitch13StatsCalculator::m_timeout),
                         MakeTimeChecker (Seconds (1)))
          .AddAttribute (
              "OutputFilename", "Filename for dumping switch statistics.",
              TypeId::ATTR_GET | TypeId::ATTR_CONSTRUCT, StringValue ("ofswitch_stats.log"),
              MakeStringAccessor (&OFSwitch13StatsCalculator::m_filename), MakeStringChecker ())
          .AddAttribute ("FlowTableDetails", "Dump individual pipeline flow table statistics.",
                         BooleanValue (false),
...
OFSwitch13StatsCalculator::HookSinks (Ptr<OFSwitch13Device> device)
{
  ...
  *m_wrapper->GetStream () << boolalpha << right << fixed << setprecision (3) << " " << setw (8)
                           << "TimeSec"
                           << " " << setw (12) << "LoaKbps"
                           << " " << setw (7) << "LoaUsag"
                           << " " << setw (7) << "Packets"
      ...
      for (size_t i = 0; i < m_device->GetNPipelineTables (); i++)
        {
          ...
          *m_wrapper->GetStream () << " " << setw (7) << field1 << " " << setw (7) << field2;
        }
    }
  ...
  *m_wrapper->GetStream () << std::endl;
  ...
  device->TraceConnectWithoutContext (
      "DatapathTimeout", MakeCallback (&OFSwitch13StatsCalculator::NotifyDatapathTimeout,
                                       Ptr<OFSwitch13StatsCalculator> (this)));
  device->TraceConnectWithoutContext ("OverloadDrop",
                                      MakeCallback (&OFSwitch13StatsCalculator::NotifyOverloadDrop,
                                                    Ptr<OFSwitch13StatsCalculator> (this)));
  device->TraceConnectWithoutContext ("MeterDrop",
                                      MakeCallback (&OFSwitch13StatsCalculator::NotifyMeterDrop,
                                                    Ptr<OFSwitch13StatsCalculator> (this)));
  device->TraceConnectWithoutContext ("TableDrop",
                                      MakeCallback (&OFSwitch13StatsCalculator::NotifyTableDrop,
                                                    Ptr<OFSwitch13StatsCalculator> (this)));
  device->TraceConnectWithoutContext (
      "PipelinePacket", MakeCallback (&OFSwitch13StatsCalculator::NotifyPipelinePacket,
                                      Ptr<OFSwitch13StatsCalculator> (this)));
...
OFSwitch13StatsCalculator::NotifyDatapathTimeout (Ptr<const OFSwitch13Device> device)
{
  ...
  m_ewmaCpuLoad = m_alpha * m_device->GetCpuLoad ().GetBitRate () + (1 - m_alpha) * m_ewmaCpuLoad;
...
OFSwitch13StatsCalculator::DumpStatistics (void)
{
  ...
  *m_wrapper->GetStream () << " " << setw (8) << Simulator::Now ().GetSeconds () << " " << setw (12)
                           << static_cast<double> (cpuLoad) / 1000 << " " << setw (7) << cpuUsage
                           << " " << setw (7) << m_packets << " " << setw (7)
                           << GetEwmaPipelineDelay ().GetMicroSeconds () << " " << setw (7)
                           << m_loadDrops << " " << setw (7) << m_meterDrops << " " << setw (7)
                           << m_tableDrops << " " << setw (7) << flowMods - m_lastFlowMods << " "
                           << setw (7) << meterMods - m_lastMeterMods << " " << setw (7)
                           << groupMods - m_lastGroupMods << " " << setw (7)
                           << packetsIn - m_lastPacketsIn << " " << setw (7)
                           << packetsOut - m_lastPacketsOut << " " << setw (7)
                           << GetEwmaSumFlowEntries () << " " << setw (7)
                           << GetAvgActFlowTableUsage () << " " << setw (7)
                           << GetEwmaMeterTableEntries () << " " << setw (7)
                           << GetAvgMeterTableUsage () << " " << setw (7)
                           << GetEwmaGroupTableEntries () << " " << setw (7)
                           << GetAvgGroupTableUsage () << " " << setw (7) << GetEwmaBufferEntries ()
