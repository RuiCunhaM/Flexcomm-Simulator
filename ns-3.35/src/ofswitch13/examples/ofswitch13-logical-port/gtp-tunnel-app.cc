...
bool
GtpTunnelApp::RecvFromLogicalPort (Ptr<Packet> packet, const Address &source, const Address &dest,
                                   uint16_t protocolNo)
{
...
GtpTunnelApp::RecvFromTunnelSocket (Ptr<Socket> socket)
{
  ...
  m_logicalPort->Receive (packet, Ipv4L3Protocol::PROT_NUMBER, Mac48Address (), Mac48Address (),
...
void
GtpTunnelApp::AddHeader (Ptr<Packet> packet, Mac48Address source, Mac48Address dest,
                         uint16_t protocolNo)
{
  NS_LOG_FUNCTION (this << packet << source << dest << protocolNo);
