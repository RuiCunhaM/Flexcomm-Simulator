...
#define ofq_error_string(rv) \
  (((rv) < OFQ_ERR_COUNT) && ((rv) >= 0) ? openflow_queue_error_strings[rv] : "Unknown error code")
...
#define ofq_prop_string(val)                                                    \
  (((val) < OFPQT_EXT_COUNT) && ((val) >= 0) ? openflow_queue_prop_strings[val] \
