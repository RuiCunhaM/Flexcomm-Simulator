...
OFSwitch13Device::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::OFSwitch13Device")
          ...
          .AddAttribute ("CpuCapacity", "CPU processing capacity (in terms of throughput).",
                         DataRateValue (DataRate ("100Gb/s")),
                         MakeDataRateAccessor (&OFSwitch13Device::m_cpuCapacity),
                         MakeDataRateChecker ())
          .AddAttribute ("DatapathId", "The unique identification of this OpenFlow switch.",
          ...
          .AddAttribute ("GroupTableSize", "The maximum number of entries allowed on group table.",
          ...
          .AddAttribute ("MeterTableSize", "The maximum number of entries allowed on meter table.",
          ...
          .AddAttribute ("PipelineTables", "The number of pipeline flow tables.",
                         TypeId::ATTR_GET | TypeId::ATTR_CONSTRUCT, UintegerValue (64),
                         MakeUintegerAccessor (&OFSwitch13Device::m_numPipeTabs),
                         MakeUintegerChecker<uint32_t> (1, (OFPTT_MAX + 1)))
          .AddAttribute ("PortList", "The list of ports associated to this switch.",
                         ObjectVectorValue (),
                         MakeObjectVectorAccessor (&OFSwitch13Device::m_ports),
                         MakeObjectVectorChecker<OFSwitch13Port> ())
          .AddAttribute ("TcamDelay", "Average time to perform a TCAM operation in pipeline.",
                         TimeValue (MicroSeconds (20)),
                         MakeTimeAccessor (&OFSwitch13Device::m_tcamDelay),
                         MakeTimeChecker (Time (0)))
          .AddAttribute ("TimeoutInterval", "The interval between timeout operations on datapath.",
                         TimeValue (MilliSeconds (100)),
                         MakeTimeAccessor (&OFSwitch13Device::m_timeout),
                         MakeTimeChecker (MilliSeconds (1), MilliSeconds (1000)))

          .AddTraceSource ("BufferExpire", "Trace source indicating an expired packet in buffer.",
          ...
          .AddTraceSource ("BufferRetrieve",
                           ...
                           "ns3::Packet::TracedCallback")
          .AddTraceSource ("BufferSave", "Trace source indicating a packet saved into buffer.",
                           MakeTraceSourceAccessor (&OFSwitch13Device::m_bufferSaveTrace),
                           "ns3::Packet::TracedCallback")
          .AddTraceSource ("MeterDrop", "Trace source indicating a packet dropped by meter band.",
          ...
          .AddTraceSource ("PipelinePacket", "Trace source indicating a packet sent to pipeline.",
...
OFSwitch13Device::ReceiveFromSwitchPort (Ptr<Packet> packet, uint32_t portNo, uint64_t tunnelId)
{
  ...
  Simulator::Schedule (m_pipeDelay, &OFSwitch13Device::SendToPipeline, this, packet, portNo,
                       tunnelId);
}
...
void
OFSwitch13Device::SendPacketToController (struct pipeline *pl, struct packet *pkt, uint8_t tableId,
                                          uint8_t reason)
{
...
void
OFSwitch13Device::DpActionsOutputPort (struct packet *pkt, uint32_t outPort, uint32_t outQueue,
...
OFSwitch13Device::DatapathTimeout (struct datapath *dp)
{
  ...
  m_pipeDelay =
      m_sumFlowEntries < 2U ? m_tcamDelay : m_tcamDelay * (int64_t) ceil (log2 (m_sumFlowEntries));
...
int
OFSwitch13Device::SendPacketInMessage (struct packet *pkt, uint8_t tableId, uint8_t reason,
                                       uint16_t maxLength, uint64_t cookie)
{
...
OFSwitch13Device::ReceiveFromController (Ptr<Packet> packet, Address from)
{
  ...
  error = ofl_msg_unpack ((uint8_t *) buffer->data, buffer->size, &msg, &senderCtrl.xid,
                          m_datapath->exp);
...
OFSwitch13Device::RemoteController::RemoteController () : m_socket (0), m_handler (0), m_remote (0)
{
  m_address = Address ();
}

OFSwitch13Device::PipelinePacket::PipelinePacket () : m_valid (false), m_packet (0)
{
}
