...
OFSwitch13Port::OFSwitch13Port ()
    : m_dpId (0), m_portNo (0), m_swPort (0), m_netDev (0), m_openflowDev (0)
{
...
OFSwitch13Port::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::OFSwitch13Port")
          ...
          .AddAttribute ("PortQueue", "The OpenFlow queue to use as the TX queue in this port.",
                         PointerValue (), MakePointerAccessor (&OFSwitch13Port::m_portQueue),
                         MakePointerChecker<OFSwitch13Queue> ())
          .AddAttribute ("QueueFactory", "The object factory for the OpenFlow queue.",
          ...
          .AddTraceSource (
              "SwitchPortRx", "Trace source indicating a packet received at this port.",
              MakeTraceSourceAccessor (&OFSwitch13Port::m_rxTrace), "ns3::Packet::TracedCallback")
          .AddTraceSource ("SwitchPortTx", "Trace source indicating a packet sent at this port.",
...
OFSwitch13Port::OFSwitch13Port (struct datapath *dp, Ptr<NetDevice> netDev,
    ...
    : m_dpId (0), m_portNo (0), m_swPort (0), m_netDev (netDev), m_openflowDev (openflowDev)
{
...
bool
OFSwitch13Port::Receive (Ptr<NetDevice> device, Ptr<const Packet> packet, uint16_t protocol,
...
OFSwitch13Port::Send (Ptr<const Packet> packet, uint32_t queueNo, uint64_t tunnelId)
{
  ...
  bool status = m_netDev->SendFrom (packetCopy, header.GetSource (), header.GetDestination (),
