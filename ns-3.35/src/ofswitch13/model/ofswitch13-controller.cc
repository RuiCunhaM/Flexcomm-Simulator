...
{
  static TypeId tid =
      TypeId ("ns3::OFSwitch13Controller")
          .SetParent<Object> ()
          .SetGroupName ("OFSwitch13")
          .AddAttribute ("Port", "Port number to listen for incoming packets.",
                         UintegerValue (6653), MakeUintegerAccessor (&OFSwitch13Controller::m_port),
...
OFSwitch13Controller::DpctlExecute (uint64_t dpId, const std::string textCmd)
{
  ...
  if ((strcmp (argv[0], "ping") == 0) || (strcmp (argv[0], "monitor") == 0) ||
      (strcmp (argv[0], "set-desc") == 0) || (strcmp (argv[0], "queue-mod") == 0) ||
...
int
OFSwitch13Controller::SendToSwitch (Ptr<const RemoteSwitch> swtch, struct ofl_msg_header *msg,
                                    uint32_t xid)
{
  NS_LOG_FUNCTION (this << swtch);
...
ofl_err
OFSwitch13Controller::HandleEchoRequest (struct ofl_msg_echo *msg, Ptr<const RemoteSwitch> swtch,
                                         uint32_t xid)
{
...
ofl_err
OFSwitch13Controller::HandleEchoReply (struct ofl_msg_echo *msg, Ptr<const RemoteSwitch> swtch,
                                       uint32_t xid)
{
  NS_LOG_FUNCTION (this << swtch << xid);
...
ofl_err
OFSwitch13Controller::HandleBarrierReply (struct ofl_msg_header *msg, Ptr<const RemoteSwitch> swtch,
                                          uint32_t xid)
{
  NS_LOG_FUNCTION (this << swtch << xid);
...
ofl_err
OFSwitch13Controller::HandleHello (struct ofl_msg_header *msg, Ptr<const RemoteSwitch> swtch,
                                   uint32_t xid)
{
  NS_LOG_FUNCTION (this << swtch << xid);
...
ofl_err
OFSwitch13Controller::HandlePacketIn (struct ofl_msg_packet_in *msg, Ptr<const RemoteSwitch> swtch,
                                      uint32_t xid)
{
  NS_LOG_FUNCTION (this << swtch << xid);
...
ofl_err
OFSwitch13Controller::HandleError (struct ofl_msg_error *msg, Ptr<const RemoteSwitch> swtch,
                                   uint32_t xid)
{
  NS_LOG_FUNCTION (this << swtch << xid);
...
/********** Private methods **********/
ofl_err
OFSwitch13Controller::HandleSwitchMsg (struct ofl_msg_header *msg, Ptr<RemoteSwitch> swtch,
                                       uint32_t xid)
{
  NS_LOG_FUNCTION (this << swtch << xid);
  ...
  if (!error)
    {
      ...
      char *msgStr = ofl_msg_to_string (msg, 0);
      NS_LOG_DEBUG ("RX from switch " << swtch->GetIpv4 () << " [dp " << swtch->GetDpId ()
                                      << "]: " << msgStr);
      free (msgStr);
...
OFSwitch13Controller::RemoteSwitch::RemoteSwitch ()
    : m_handler (0), m_ctrlApp (0), m_dpId (0), m_role (OFPCR_ROLE_EQUAL)
{
  m_address = Address ();
}
...
OFSwitch13Controller::EchoInfo::EchoInfo (Ptr<const RemoteSwitch> swtch)
    : m_waiting (true), m_send (Simulator::Now ()), m_swtch (swtch)
{
}
