...
void
FqCobaltQueueDiscEcnMarking::AddPacket (Ptr<FqCobaltQueueDisc> queue, Ipv4Header hdr,
                                        u_int32_t nPkt, u_int32_t nPktEnqueued,
...
FqCobaltQueueDiscL4sMode::DoRun (void)
{
  ...
  NS_TEST_EXPECT_MSG_EQ (q1->GetStats ().GetNMarkedPackets (CobaltQueueDisc::FORCED_MARK), 2,
                         ...
                         "Cobalt keeps drop_next as now and the next packet is marked. With second "
                         "dequeue count increases to 2, drop_next becomes now plus around"
                         "70ms which is less than the running time(140), and as the queue delay is "
                         "persistantly higher than 5ms, second packet is marked.");
  NS_TEST_EXPECT_MSG_EQ (q1->GetStats ().GetNDroppedPackets (CobaltQueueDisc::TARGET_EXCEEDED_DROP),
                         0, "There should not be any dropped packets");
