...
private:
  ...
  void AddPacket (Ptr<FqCoDelQueueDisc> queue, Ipv4Header hdr, u_int32_t nPkt,
                  u_int32_t nPktEnqueued, u_int32_t nQueueFlows);
...
void
FqCoDelQueueDiscECNMarking::AddPacket (Ptr<FqCoDelQueueDisc> queue, Ipv4Header hdr, u_int32_t nPkt,
                                       u_int32_t nPktEnqueued, u_int32_t nQueueFlows)
{
  Address dest;
  Ptr<Packet> p = Create<Packet> (100);
...
FqCoDelQueueDiscECNMarking::DoRun (void)
{
  ...
  NS_TEST_EXPECT_MSG_EQ (q0->GetStats ().GetNMarkedPackets (CoDelQueueDisc::TARGET_EXCEEDED_MARK),
                         ...
                         "test's default target value 5ms. Sojourn time has just gone above target "
                         "from below, need to stay above for at"
                         "least q->interval before packet can be dropped. Second packet dequeues "
                         "at 220ms which is greater than last dequeue"
                         "time plus q->interval(test default 100ms) so the packet is marked. Third "
                         "packet dequeues at 330ms and the sojourn"
  ...
  NS_TEST_EXPECT_MSG_EQ (
      ...
      "with 20 packets, total bytes in the queue = 120 * 20 = 2400. First packet dequeues at 110ms "
      ...
      "and the sojourn time stayed above the target and dropnext value is less than 320 hence the "
      "packet is dropped and next"
      "packet is dequeued. 6th packet dequeues at 440ms and 2 more packets are dropped as dropnext "
      "value is increased twice."
...
FqCoDelQueueDiscL4sMode::DoRun (void)
{
  ...
  NS_TEST_EXPECT_MSG_EQ (
      ...
      "4th packet is enqueued at 2ms and dequeued at 4ms hence the delay of 2ms which not greater "
      "than CE threshold"
      "5th packet is enqueued at 2.5ms and dequeued at 5ms hence the delay of 2.5ms and subsequent "
      "packet also do have delay"
  ...
  NS_TEST_EXPECT_MSG_EQ (
      ...
      "2nd ECT1 packet is enqueued at 1.5ms and dequeued at 3ms hence the delay of 1.5ms which not "
      "greater than CE threshold"
      "3rd packet is enqueued at 2.5ms and dequeued at 5ms hence the delay of 2.5ms and subsequent "
      "packet also do have delay"
