...
public:
  static void
  Sink (T1 a1, T2 a2, T3 a3, T4 a4, T5 a5)
  {
    SinkIt (5);
  }
};
...
#define CHECK(U, T1, T2, T3, T4, T5) CreateObject<Checker<T1, T2, T3, T4, T5>> ()->Invoke<U> ()
  ...
  CHECK (dsr::DsrOptionSRHeader::TracedCallback, const dsr::DsrOptionSRHeader &, empty, empty,
         empty, empty);

  CHECK (EpcUeNas::StateTracedCallback, EpcUeNas::State, EpcUeNas::State, empty, empty, empty);

  CHECK (Ipv4L3Protocol::DropTracedCallback, const Ipv4Header &, Ptr<const Packet>,
         Ipv4L3Protocol::DropReason, Ptr<Ipv4>, uint32_t);

  CHECK (Ipv4L3Protocol::SentTracedCallback, const Ipv4Header &, Ptr<const Packet>, uint32_t, empty,
         empty);

  CHECK (Ipv4L3Protocol::TxRxTracedCallback, Ptr<const Packet>, Ptr<Ipv4>, uint32_t, empty, empty);

  CHECK (Ipv6L3Protocol::DropTracedCallback, const Ipv6Header &, Ptr<const Packet>,
         Ipv6L3Protocol::DropReason, Ptr<Ipv6>, uint32_t);

  CHECK (Ipv6L3Protocol::SentTracedCallback, const Ipv6Header &, Ptr<const Packet>, uint32_t, empty,
         empty);

  CHECK (Ipv6L3Protocol::TxRxTracedCallback, Ptr<const Packet>, Ptr<Ipv6>, uint32_t, empty, empty);

  CHECK (LrWpanMac::SentTracedCallback, Ptr<const Packet>, uint8_t, uint8_t, empty, empty);

  CHECK (LrWpanMac::StateTracedCallback, LrWpanMacState, LrWpanMacState, empty, empty, empty);

  CHECK (LrWpanPhy::StateTracedCallback, Time, LrWpanPhyEnumeration, LrWpanPhyEnumeration, empty,
         empty);
  ...
  */
  ...
  CHECK (LteEnbPhy::ReportUeSinrTracedCallback, uint16_t, uint16_t, double, uint8_t, empty);

  CHECK (LteEnbPhy::ReportInterferenceTracedCallback, uint16_t, Ptr<SpectrumValue>, empty, empty,
         empty);

  CHECK (LteEnbRrc::ConnectionHandoverTracedCallback, uint64_t, uint16_t, uint16_t, empty, empty);

  CHECK (LteEnbRrc::HandoverStartTracedCallback, uint64_t, uint16_t, uint16_t, uint16_t, empty);

  CHECK (LteEnbRrc::NewUeContextTracedCallback, uint16_t, uint16_t, empty, empty, empty);

  CHECK (LteEnbRrc::ReceiveReportTracedCallback, uint64_t, uint16_t, uint16_t,
         LteRrcSap::MeasurementReport, empty);

  CHECK (LtePdcp::PduRxTracedCallback, uint16_t, uint8_t, uint32_t, uint64_t, empty);

  CHECK (LtePdcp::PduTxTracedCallback, uint16_t, uint8_t, uint32_t, empty, empty);
  ...
  CHECK (LteUeRrc::CellSelectionTracedCallback, uint64_t, uint16_t, empty, empty, empty);
  ...
  CHECK (Mac48Address::TracedCallback, Mac48Address, empty, empty, empty, empty);

  CHECK (MobilityModel::TracedCallback, Ptr<const MobilityModel>, empty, empty, empty, empty);

  CHECK (olsr::RoutingProtocol::PacketTxRxTracedCallback, const olsr::PacketHeader &,
         const olsr::MessageList &, empty, empty, empty);

  CHECK (olsr::RoutingProtocol::TableChangeTracedCallback, uint32_t, empty, empty, empty, empty);

  CHECK (Packet::AddressTracedCallback, Ptr<const Packet>, const Address &, empty, empty, empty);

  CHECK (Packet::Mac48AddressTracedCallback, Ptr<const Packet>, Mac48Address, empty, empty, empty);

  CHECK (Packet::SinrTracedCallback, Ptr<const Packet>, double, empty, empty, empty);

  CHECK (Packet::SizeTracedCallback, uint32_t, uint32_t, empty, empty, empty);

  CHECK (Packet::TracedCallback, Ptr<const Packet>, empty, empty, empty, empty);

  CHECK (PacketBurst::TracedCallback, Ptr<const PacketBurst>, empty, empty, empty, empty);

  CHECK (dot11s::PeerManagementProtocol::LinkOpenCloseTracedCallback, Mac48Address, Mac48Address,
         empty, empty, empty);

  CHECK (PhyReceptionStatParameters::TracedCallback, PhyReceptionStatParameters, empty, empty,
         empty, empty);

  CHECK (PhyTransmissionStatParameters::TracedCallback, PhyTransmissionStatParameters, empty, empty,
         empty, empty);

  CHECK (SixLowPanNetDevice::DropTracedCallback, SixLowPanNetDevice::DropReason, Ptr<const Packet>,
         Ptr<SixLowPanNetDevice>, uint32_t, empty);

  CHECK (SixLowPanNetDevice::RxTxTracedCallback, Ptr<const Packet>, Ptr<SixLowPanNetDevice>,
         uint32_t, empty, empty);

  CHECK (SpectrumChannel::LossTracedCallback, Ptr<const SpectrumPhy>, Ptr<const SpectrumPhy>,
         double, empty, empty);

  CHECK (SpectrumValue::TracedCallback, Ptr<SpectrumValue>, empty, empty, empty, empty);

  CHECK (TimeSeriesAdaptor::OutputTracedCallback, double, double, empty, empty, empty);

  CHECK (UanMac::PacketModeTracedCallback, Ptr<const Packet>, UanTxMode, empty, empty, empty);

  CHECK (UanMacCw::QueueTracedCallback, Ptr<const Packet>, uint16_t, empty, empty, empty);

  CHECK (UanMacRc::QueueTracedCallback, Ptr<const Packet>, uint32_t, empty, empty, empty);

  CHECK (UanNetDevice::RxTxTracedCallback, Ptr<const Packet>, Mac8Address, empty, empty, empty);

  CHECK (UanPhy::TracedCallback, Ptr<const Packet>, double, UanTxMode, empty, empty);

  CHECK (UeManager::StateTracedCallback, uint64_t, uint16_t, uint16_t, UeManager::State,
         UeManager::State);

  CHECK (WifiMacHeader::TracedCallback, const WifiMacHeader &, empty, empty, empty, empty);

  CHECK (WifiPhyStateHelper::RxEndErrorTracedCallback, Ptr<const Packet>, double, empty, empty,
         empty);

  CHECK (WifiPhyStateHelper::RxOkTracedCallback, Ptr<const Packet>, double, WifiMode, WifiPreamble,
         empty);

  CHECK (WifiPhyStateHelper::StateTracedCallback, Time, Time, WifiPhyState, empty, empty);

  CHECK (WifiPhyStateHelper::TxTracedCallback, Ptr<const Packet>, WifiMode, WifiPreamble, uint8_t,
         empty);

  CHECK (WifiRemoteStationManager::PowerChangeTracedCallback, double, double, Mac48Address, empty,
         empty);
