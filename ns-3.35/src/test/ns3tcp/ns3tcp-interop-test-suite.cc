...
const uint32_t PCAP_LINK_TYPE =
    1187373553; // Some large random number -- we use to verify data was written by this program
...
Ns3TcpInteroperabilityTestCase::Ns3TcpInteroperabilityTestCase ()
    : TestCase ("Check to see that the ns-3 TCP can work with liblinux2.6.26.so"),
...
void
Ns3TcpInteroperabilityTestCase::Ipv4L3Tx (std::string context, Ptr<const Packet> packet,
  ...
  if (m_writeVectors)
    {
      ...
      m_pcapFile.Write (uint32_t (tMicroSeconds / 1000000), uint32_t (tMicroSeconds % 1000000), p);
    }
  else
    {
      //
