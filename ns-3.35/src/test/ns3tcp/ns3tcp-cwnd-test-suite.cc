...
void
SimpleSource::Setup (Ptr<Socket> socket, Address address, uint32_t packetSize, uint32_t nPackets,
                     DataRate dataRate)
{
  m_socket = socket;
  m_peer = address;
...
Ns3TcpCwndTestCase1::Ns3TcpCwndTestCase1 ()
    : TestCase ("Check to see that the ns-3 TCP congestion window works as expected against "
                "liblinux2.6.26.so"),
...
Ns3TcpCwndTestCase1::DoRun (void)
{
  ...
  ns3TcpSocket->TraceConnectWithoutContext ("CongestionWindowInflated",
  ...
  NS_TEST_ASSERT_MSG_EQ (m_responses.GetN (), N_EVENTS,
  ...
  for (uint32_t i = 1, from = MSS, to = MSS * 2; i < N_EVENTS; ++i, from += MSS, to += MSS)
    {
      ...
      NS_TEST_ASSERT_MSG_EQ (event.m_oldCwnd, from,
                             "Wrong old cwnd value in cwnd change event " << i);
      NS_TEST_ASSERT_MSG_EQ (event.m_newCwnd, to,
...
{
  NS_LOG_DEBUG ("Cwnd NOT INFLATED change event " << m_responses.GetN () << " at "
                                                  << Now ().As (Time::S) << " " << oldCwnd << " "
                                                  << newCwnd);
}
...
Ns3TcpCwndTestCase2::DoRun (void)
{
  ...
  ns3TcpSocket->TraceConnectWithoutContext ("CongestionWindowInflated",
                                            MakeCallback (&Ns3TcpCwndTestCase2::CwndChange, this));
  ns3TcpSocket->TraceConnectWithoutContext (
      "CongestionWindow", MakeCallback (&Ns3TcpCwndTestCase2::CwndChangeNotInflated, this));
  ...
  NS_TEST_ASSERT_MSG_EQ (event.m_newCwnd, (MSS * 15) / 2,
  ...
  NS_TEST_ASSERT_MSG_EQ (m_responses.Get (16).m_newCwnd, (MSS * 19) / 2,
                         "Wrong new cwnd value in cwnd change event " << 16);

  // partial ack again of 3 segments after one more acks, cwnd reset to 7.5
  NS_TEST_ASSERT_MSG_EQ (m_responses.Get (18).m_newCwnd, (MSS * 15) / 2,
  ...
  NS_TEST_ASSERT_MSG_EQ (m_responses.Get (25).m_newCwnd, 2412,
                         "Wrong new cwnd value in cwnd change event " << 25);
  ...
  for (uint32_t i = 26; i < N_EVENTS; ++i)
    {
      ...
      NS_TEST_ASSERT_MSG_EQ (m_responses.Get (i).m_newCwnd, cwnd,
                             "Wrong new cwnd value in cwnd change event " << i);
    }
  NS_LOG_DEBUG ("Reading out the cwnd event log");
  for (uint32_t i = 0; i < N_EVENTS; ++i)
    {
      NS_LOG_DEBUG ("i: " << i << " newCwnd: " << m_responses.Get (i).m_newCwnd
                          << " newCwnd segments "
                          << static_cast<double> (m_responses.Get (i).m_newCwnd) / MSS);
    }
}

void
Ns3TcpCwndTestCase2::VerifyCwndRun (uint32_t beginIdx, uint32_t endIdx, uint32_t initialCwnd,
                                    uint32_t mss)
{
...
Ns3TcpCwndTestSuite::Ns3TcpCwndTestSuite () : TestSuite ("ns3-tcp-cwnd", SYSTEM)
{
