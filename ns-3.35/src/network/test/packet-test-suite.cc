...
public:
  ...
  virtual void
  Serialize (Buffer::Iterator iter) const
  {
    for (uint32_t i = 0; i < N; ++i)
      {
        iter.WriteU8 (N);
  ...
  virtual uint32_t
  Deserialize (Buffer::Iterator iter)
  {
    for (uint32_t i = 0; i < N; ++i)
      {
...
#define CHECK_DATA(p, n, ...) DoCheckData (p, __FILE__, __LINE__, n, __VA_ARGS__)
  ...
  while (i.HasNext () && j < expected.size ())
    {
      ...
      NS_TEST_EXPECT_MSG_EQ_INTERNAL (item.GetTypeId ().GetName (), oss.str (), "trivial", file,
                                      line);
