...
public:
   ...
   */
  typedef void (*CycleCallback) (Time now, Time delay, uint32_t numRts, uint32_t totalBytes,
...
private:
  ...
  uint32_t
      m_totalRate; //!< Total available channel rate in bps (for a single channel, without splitting reservation channel).
   ...
   */
  double ComputeAlpha (uint32_t totalFrames, uint32_t totalBytes, uint32_t n, uint32_t a,
                       double deltaK);
  /**
