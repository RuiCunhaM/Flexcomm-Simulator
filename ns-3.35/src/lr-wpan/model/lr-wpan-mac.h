...
struct MlmeStartRequestParams
{
  ...
  uint32_t
      m_logChPage; //!< Logical channel page on which to start using the new superframe configuration.
  uint32_t
      m_startTime; //!< Time at which to begin transmitting beacons (Used by Coordinator not PAN Coordinators). The time is specified in symbols.
  uint8_t
      m_bcnOrd; //!< Beacon Order, Used to calculate the beacon interval, a value of 15 indicates no periodic beacons will be transmitted.
  ...
  bool
      m_battLifeExt; //!< Flag indicating whether or not the Battery life extension (BLE) features are used.
  bool
      m_coorRealgn; //!< True if a realignment request command is to be transmitted prior changing the superframe.
};
/**
 * \ingroup lr-wpan
 *
...
{
  MlmeSyncRequestParams () : m_logCh (), m_trackBcn (false)
  {
  }
  uint8_t m_logCh; //!< The channel number on which to attempt coordinator synchronization.
  bool
      m_trackBcn; //!< True if the mlme sync with the next beacon and attempts to track future beacons. False if mlme sync only the next beacon.
};
/**
 * \ingroup lr-wpan
 *
...
struct MlmePollRequestParams
{
  MlmePollRequestParams ()
      : m_coorAddrMode (SHORT_ADDR), m_coorPanId (0), m_coorShortAddr (), m_coorExtAddr ()
  {
  }
  LrWpanAddressMode
      m_coorAddrMode; //!< The addressing mode of the coordinator to which the pool is intended.
