  ...
  else if (b2 == TX_OPTION_INDIRECT)
    {
      ...
      if (m_txQueue.size () == m_txQueue.max_size ())
      ...
      else
        {
          ...
          std::cout << "Indirect Transmission Pushed | Elements in the queue: "
                    << m_indTxQueue.size () << " "
...
LrWpanMac::MlmeStartRequest (MlmeStartRequestParams params)
{
  ...
  if ((params.m_logCh > 26) || (params.m_bcnOrd > 15) || (params.m_sfrmOrd > params.m_bcnOrd))
    {
...
LrWpanMac::SendOneBeacon ()
{
  ...
  NS_LOG_DEBUG ("Outgoing superframe Active Portion (Beacon + CAP + CFP): " << m_superframeDuration
                                                                            << " symbols");
              ...
              if (receivedMacHdr.IsBeacon ())
                {
                  ...
                  m_incomingBeaconInterval =
                      ((uint32_t) 1 << m_incomingBeaconOrder) * aBaseSuperframeDuration;
                  m_incomingSuperframeDuration =
                      aBaseSuperframeDuration * ((uint32_t) 1 << m_incomingSuperframeOrder);
