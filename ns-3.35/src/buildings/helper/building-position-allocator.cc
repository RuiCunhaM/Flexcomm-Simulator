...
SameRoomPositionAllocator::SameRoomPositionAllocator (NodeContainer c) : m_nodes (c)
{
  m_rand = CreateObject<UniformRandomVariable> ();
...
FixedRoomPositionAllocator::FixedRoomPositionAllocator (uint32_t x, uint32_t y, uint32_t z,
                                                        Ptr<Building> pbtr)
{
  m_rand = CreateObject<UniformRandomVariable> ();
  roomx = x;
