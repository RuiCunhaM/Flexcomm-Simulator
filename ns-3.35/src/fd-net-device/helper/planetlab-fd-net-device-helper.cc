  ...
  if (pid == 0)
  ...
  else
    {
      ...
      if (WIFEXITED (st))
        {
          ...
          NS_ABORT_MSG_IF (exitStatus != 0, "PlanetLabFdNetDeviceHelper::CreateFileDescriptor(): "
                                            "socket creator exited normally with status "
                                                << exitStatus);
        }
      else
        {
          NS_FATAL_ERROR ("PlanetLabFdNetDeviceHelper::CreateFileDescriptor(): socket creator "
                          "exited abnormally");
        }
      ...
      NS_ABORT_MSG_IF (bytesRead != sizeof (int),
                       "PlanetLabFdNetDeviceHelper::CreateFileDescriptor(): Wrong byte count from "
                       "socket creator");
