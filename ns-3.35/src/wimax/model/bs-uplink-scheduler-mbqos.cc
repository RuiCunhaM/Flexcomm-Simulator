...
UplinkSchedulerMBQoS::UplinkSchedulerMBQoS (Time time) : m_windowInterval (time)
{
}
...
UplinkSchedulerMBQoS::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::UplinkSchedulerMBQoS")
                          ...
                          .AddAttribute ("WindowInterval", "The time to wait to reset window",
...
void
UplinkSchedulerMBQoS::GetChannelDescriptorsToUpdate (bool &updateDcd, bool &updateUcd,
                                                     bool &sendDcd, bool &sendUcd)
{
...
void
UplinkSchedulerMBQoS::AddUplinkAllocation (OfdmUlMapIe &ulMapIe, const uint32_t &allocationSize,
  ...
  while ((availableSymbols) && (!m_uplinkJobs_high.empty ()))
    {
      ...
      if (reqType == UNICAST_POLLING)
        {
          ServiceUnsolicitedGrants (ssRecord, schedulingType, ulMapIe, modulationType,
      ...
      else if (reqType == DATA)
        {
          ...
          ServiceBandwidthRequestsBytes (serviceFlow, schedulingType, ulMapIe, modulationType,
                                         symbolsToAllocation, availableSymbols, allocSizeBytes);
        }
  ...
  while ((availableSymbols) && (!m_uplinkJobs_inter.empty ()))
    {
      ...
      if (reqType == DATA)
        {
          ServiceBandwidthRequests (ssRecord, schedulingType, ulMapIe, modulationType,
                                    symbolsToAllocation, availableSymbols);
        }
      else
        {
...
UplinkSchedulerMBQoS::ServiceBandwidthRequestsBytes (
    ServiceFlow *serviceFlow, enum ServiceFlow::SchedulingType schedulingType, OfdmUlMapIe &ulMapIe,
    const WimaxPhy::ModulationType modulationType, uint32_t &symbolsToAllocation,
    uint32_t &availableSymbols, uint32_t allocationSizeBytes)
{
...
Ptr<UlJob>
UplinkSchedulerMBQoS::CreateUlJob (SSRecord *ssRecord, enum ServiceFlow::SchedulingType schedType,
                                   ReqType reqType)
{
...
UplinkSchedulerMBQoS::CheckMinimumBandwidth (uint32_t &availableSymbols)
{
  ...
  for (std::list<Ptr<UlJob>>::const_iterator iter = m_uplinkJobs_inter.begin ();
       iter != m_uplinkJobs_inter.end (); ++iter)
    {
      Ptr<UlJob> job = *iter;
      // SSRecord ssRecord = job->GetSsRecord();
      ServiceFlow *serviceFlow = job->GetServiceFlow ();
                                                ...
                                                uint32_t &availableSymbols)
{
  ...
  for (std::vector<ServiceFlow *>::iterator iter = serviceFlows.begin ();
       iter != serviceFlows.end (); ++iter)
    {
      if (!ServiceBandwidthRequests (*iter, schedulingType, ulMapIe, modulationType,
                                     symbolsToAllocation, availableSymbols))
        {
          break;
        }
...
UplinkSchedulerMBQoS::SetupServiceFlow (SSRecord *ssRecord, ServiceFlow *serviceFlow)
{
  ...
  uint32_t bytesPerFrame =
      (uint32_t (
          (double) (bitsPerSecond) *GetBs ()->GetPhy ()->GetFrameDuration ().GetSeconds ())) /
      8;
