...
SSLinkManager::StartSynchronizing (void)
{
  ...
  m_ss->SetTimer (Simulator::Schedule (m_ss->GetIntervalT21 (), &SSLinkManager::StartScanning, this,
...
SSLinkManager::StartContentionResolution (void)
{
  NS_ASSERT_MSG (
      ...
      "SS: Can not start connection resolution: The SS state should be SS_STATE_WAITING_RNG_RSP or "
      "SS_STATE_WAITING_REG_RANG_INTRVL or SS_STATE_ADJUSTING_PARAMETERS");
      ...
      if (m_rangingBO == 0)
        {
          ...
          Simulator::Schedule (timeToAllocation, &SSLinkManager::SendRangingRequest, this,
...
{
  m_rangingCW =
      (uint8_t) std::pow ((double) 2, (double) m_ss->GetCurrentUcd ().GetRangingBackoffStart ()) -
      1;
}
...
{
  m_ss->SetTimer (Simulator::Schedule (interval, &SSLinkManager::StartScanning, this, eventType,
                                       deleteUlParameters),
                  eventId);
}
