...
DcdChannelEncodings::DcdChannelEncodings (void) : m_bsEirp (0), m_eirXPIrMax (0), m_frequency (0)
{
}
...
OfdmDcdChannelEncodings::DoRead (Buffer::Iterator start)
{
  ...
  ReadFrom (
      i,
      m_baseStationId); // length (6) shall also be written in packet instead of hard coded, see ARP example
...
Dcd::Dcd (void) : m_reserved (0), m_configurationChangeCount (0), m_nrDlBurstProfiles (0)
{
}
...
Dcd::GetSerializedSize (void) const
{
  ...
  for (std::vector<OfdmDlBurstProfile>::const_iterator iter = m_dlBurstProfiles.begin ();
       iter != m_dlBurstProfiles.end (); ++iter)
    {
...
OfdmDlMapIe::OfdmDlMapIe (void) : m_cid (), m_diuc (0), m_preamblePresent (0), m_startTime (0)
{
}
...
DlMap::DlMap (void) : m_dcdCount (0), m_baseStationId (Mac48Address ("00:00:00:00:00:00"))
{
}
