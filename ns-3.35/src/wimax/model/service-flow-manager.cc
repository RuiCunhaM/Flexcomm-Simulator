...
TypeId
ServiceFlowManager::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::ServiceFlowManager").SetParent<Object> ().SetGroupName ("Wimax");
  return tid;
}
...
ServiceFlow *
ServiceFlowManager::DoClassify (Ipv4Address srcAddress, Ipv4Address dstAddress, uint16_t srcPort,
                                uint16_t dstPort, uint8_t proto, ServiceFlow::Direction dir) const
{
  for (std::vector<ServiceFlow *>::iterator iter = m_serviceFlows->begin ();
       iter != m_serviceFlows->end (); ++iter)
    {
...
ServiceFlowManager::AreServiceFlowsAllocated (std::vector<ServiceFlow *> serviceFlowVector)
{
  for (std::vector<ServiceFlow *>::const_iterator iter = serviceFlowVector.begin ();
       iter != serviceFlowVector.end (); ++iter)
    {
