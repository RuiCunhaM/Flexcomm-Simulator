...
ManagementMessageType::ManagementMessageType (void) : m_type (~0)
{
}
...
RngRsp::Deserialize (Buffer::Iterator start)
{
  ...
  ReadFrom (
      i,
      m_macAddress); // length (6) shall also be written in packet instead of hard coded, see ARP example
...
DsaReq::DsaReq (void)
    : m_transactionId (0), m_sfid (0), m_cid (), m_serviceFlow (ServiceFlow::SF_DIRECTION_DOWN)
{
}
...
DsaRsp::DsaRsp (void) : m_transactionId (0), m_confirmationCode (0), m_sfid (0), m_cid ()
{
}
...
DsaAck::DsaAck (void) : m_transactionId (0), m_confirmationCode (0)
{
}
