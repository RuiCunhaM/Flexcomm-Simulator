...
TypeId
WimaxConnection::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::WimaxConnection")
          ...
          .AddAttribute ("Type", "Connection type", EnumValue (Cid::INITIAL_RANGING),
                         MakeEnumAccessor (&WimaxConnection::GetType),
                         MakeEnumChecker (Cid::BROADCAST, "Broadcast", Cid::INITIAL_RANGING,
                                          "InitialRanging", Cid::BASIC, "Basic", Cid::PRIMARY,
                                          "Primary", Cid::TRANSPORT, "Transport", Cid::MULTICAST,
                                          "Multicast", Cid::PADDING, "Padding"))
...
WimaxConnection::WimaxConnection (Cid cid, enum Cid::Type type)
    : m_cid (cid), m_cidType (type), m_queue (CreateObject<WimaxMacQueue> (1024)), m_serviceFlow (0)
{
}
...
bool
WimaxConnection::Enqueue (Ptr<Packet> packet, const MacHeaderType &hdrType,
...
WimaxConnection::GetFragmentsQueue (void) const
{
