...
TypeId
SimpleOfdmWimaxPhy::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::SimpleOfdmWimaxPhy")
          ...
          .AddTraceSource ("PhyRxEnd",
                           "Trace source indicating a packet has been completely received from the "
                           "channel medium by the device",
...
SimpleOfdmWimaxPhy::Send (SendParams *params)
{
  ...
  Send (o_params->GetBurst (), (WimaxPhy::ModulationType) o_params->GetModulationType (),
...
void
SimpleOfdmWimaxPhy::Send (Ptr<PacketBurst> burst, WimaxPhy::ModulationType modulationType,
                          uint8_t direction)
{
                                            ...
                                            uint8_t direction)
{
  ...
  channel->Send (m_blockTime, m_currentBurstSize, this, isFirstBlock, isLastFecBlock,
                 GetTxFrequency (), modulationType, direction, m_txPower, m_currentBurst);

  m_nrRemainingBlocksToSend--;
  Simulator::Schedule (m_blockTime, &SimpleOfdmWimaxPhy::EndSendFecBlock, this, modulationType,
                       direction);
}
...
void
SimpleOfdmWimaxPhy::StartReceive (uint32_t burstSize, bool isFirstBlock, uint64_t frequency,
      ...
      if (frequency == GetRxFrequency ())
        {
          ...
          Simulator::Schedule (m_blockTime, &SimpleOfdmWimaxPhy::EndReceiveFecBlock, this,
                               burstSize, modulationType, direction, drop, burst);
...
void
SimpleOfdmWimaxPhy::EndReceiveFecBlock (uint32_t burstSize, WimaxPhy::ModulationType modulationType,
