...
{
  for (std::vector<SSRecord *>::iterator iter1 = m_ssRecords->begin ();
       iter1 != m_ssRecords->end (); ++iter1)
    {
      ...
      if (ssRecord->GetBasicCid () == cid || ssRecord->GetPrimaryCid () == cid)
      ...
      else
        {
          ...
          for (std::vector<ServiceFlow *>::iterator iter2 = sf.begin (); iter2 != sf.end ();
               ++iter2)
            {
