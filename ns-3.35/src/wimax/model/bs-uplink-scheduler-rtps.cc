...
void
UplinkSchedulerRtps::GetChannelDescriptorsToUpdate (bool &updateDcd, bool &updateUcd, bool &sendDcd,
                                                    bool &sendUcd)
{
...
void
UplinkSchedulerRtps::AddUplinkAllocation (OfdmUlMapIe &ulMapIe, const uint32_t &allocationSize,
      ...
      else
        {
          ...
          if (ssRecord->GetRangingStatus () == WimaxNetDevice::RANGING_STATUS_SUCCESS &&
          ...
          else
            {
              ...
              ServiceUnsolicitedGrants (ssRecord, ServiceFlow::SF_TYPE_UGS, ulMapIe, modulationType,
              ...
              if (availableSymbols)
                {
                  ServiceUnsolicitedGrants (ssRecord, ServiceFlow::SF_TYPE_RTPS, ulMapIe,
                                            modulationType, symbolsToAllocation, availableSymbols);
                }
              // allocate unicast polls for nrtPS flows if bandwidth is available
              if (availableSymbols)
                {
                  ServiceUnsolicitedGrants (ssRecord, ServiceFlow::SF_TYPE_NRTPS, ulMapIe,
                                            modulationType, symbolsToAllocation, availableSymbols);
                }
          ...
          if (currentTime - record->GetGrantTimeStamp () > Seconds (1))
            {
              ...
              if (bps < serviceFlow->GetMinReservedTrafficRate ())
                {
                  ServiceBandwidthRequests (serviceFlow, schedulingType, ulMapIe, modulationType,
                                            symbolsToAllocation, availableSymbols);
