...
SystemThread::SystemThread (Callback<void> callback) : m_callback (callback)
{
...
SystemThread::Start (void)
{
  ...
  int rc = pthread_create (&m_thread, NULL, &SystemThread::DoRun, (void *) this);

  if (rc)
    {
      NS_FATAL_ERROR ("pthread_create failed: " << rc << "=\"" << strerror (rc) << "\".");
    }
}
...
SystemThread::Join (void)
{
  ...
  if (rc)
    {
      NS_FATAL_ERROR ("pthread_join failed: " << rc << "=\"" << strerror (rc) << "\".");
    }
}
