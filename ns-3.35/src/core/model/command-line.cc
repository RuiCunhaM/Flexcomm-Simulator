...
CommandLine::PrintDoxygenUsage (void) const
{
  ...
  os << "/**\n \\file " << m_shortName << ".cc\n"
     << "<h3>Usage</h3>\n"
     << "<code>$ ./waf --run \"" << m_shortName << (m_options.size () ? " [Program Options]" : "")
...
CommandLine::PrintGlobals (std::ostream &os) const
{
  ...
  for (std::vector<std::string>::const_iterator it = globals.begin (); it < globals.end (); ++it)
    {
      os << *it;
    }
}
  ...
  for (uint32_t i = 0; i < tid.GetAttributeN (); ++i)
    {
      ...
      ss << info.initialValue->SerializeToString (info.checker) << "]" << std::endl;
      ss << "        " << info.help << std::endl;
      attributes.push_back (ss.str ());
    }
  std::sort (attributes.begin (), attributes.end ());
  for (std::vector<std::string>::const_iterator it = attributes.begin (); it < attributes.end ();
       ++it)
    {
      os << *it;
    }
...
CommandLine::PrintGroup (std::ostream &os, const std::string &group) const
{
  ...
  for (std::vector<std::string>::const_iterator it = groupTypes.begin (); it < groupTypes.end ();
       ++it)
    {
      os << *it;
    }
...
CommandLine::PrintGroups (std::ostream &os) const
{
  ...
  for (std::set<std::string>::const_iterator k = groups.begin (); k != groups.end (); ++k)
    {
      os << "    " << *k << std::endl;
    }
}
