...
public:
  ...
  void Check (const long double value, const int64_t intPart, const uint64_t lo);
...
Int64x64DoubleTestCase::Int64x64DoubleTestCase () : TestCase ("Construct from floating point.")
{
}

void
Int64x64DoubleTestCase::Check (const long double value, const int64_t intPart, const uint64_t lo)
{
  ...
  if (delta > int64x64_t ())
    {
      ...
      if (delta == tolerance)
      ...
      else
        {
          ...
          std::cout << GetParent ()->GetName () << std::left << std::setw (43) << "         +/-"
                    << std::right << Printer (tolerance) << std::endl;
        }
...
Int64x64DoubleTestCase::Check (const int64_t intPart)
{
  ...
  std::cout << GetParent ()->GetName () << " Double: "
            << "integer: " << intPart << ": delta count: " << m_deltaCount
...
Int64x64DoubleTestCase::DoRun (void)
{
  ...
  std::cout << GetParent ()->GetName () << " Double: " << std::endl;
