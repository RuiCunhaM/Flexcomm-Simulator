...
bool
operator!= (const ValueClassTest &a, const ValueClassTest &b)
{
  NS_UNUSED (a);
  NS_UNUSED (b);
  return true;
...
std::ostream &
operator<< (std::ostream &os, ValueClassTest v)
{
  NS_UNUSED (v);
  return os;
}
std::istream &
operator>> (std::istream &is, ValueClassTest &v)
{
  NS_UNUSED (v);
  return is;
}
...
private:
  ...
  void
  NotifySource1 (int8_t old, int8_t n)
  {
    NS_UNUSED (old);
    m_got1 = n;
  }
...
private:
  ...
  void
  NotifySource2 (double a, int b, float c)
  {
    NS_UNUSED (b);
    NS_UNUSED (c);
    m_got2 = a;
