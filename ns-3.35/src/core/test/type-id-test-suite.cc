  ...
  for (uint16_t i = 0; i < nids; ++i)
    {
      ...
      cout << "  0x" << std::setfill ('0') << std::hex << std::setw (8) << tid.GetHash ()
           << std::dec << std::setfill (' ') << "    " << tid.GetName () << endl;
...
DeprecatedAttributeTestCase::DoRun (void)
{
  ...
  cerr << suite << "lookup new attribute:"
       << (ainfo.supportLevel == TypeId::SUPPORTED ? "supported" : "error") << endl;
  ...
  cerr << suite << "lookup old attribute:"
       << (ainfo.supportLevel == TypeId::DEPRECATED ? "deprecated" : "error") << endl;
  ...
  cerr << suite << "lookup new trace source:"
       << (tinfo.supportLevel == TypeId::SUPPORTED ? "supported" : "error") << endl;
  ...
  cerr << suite << "lookup old trace source:"
       << (tinfo.supportLevel == TypeId::DEPRECATED ? "deprecated" : "error") << endl;
}
...
LookupTimeTestCase::DoSetup (void)
{
  ...
  cout << suite << "Lookup time: reps: " << REPETITIONS << ", num TypeId's: " << nids << endl;
}
...
LookupTimeTestCase::Report (const std::string how, const uint32_t delta) const
{
  ...
  cout << suite << "Lookup time: by " << how << ": "
       << "ticks: " << delta << "\tper: " << per << " microsec/lookup" << endl;
}
