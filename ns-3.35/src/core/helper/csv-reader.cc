...
CsvReader::CsvReader (const std::string &filepath, char delimiter /* =',' */)
    : m_delimiter (delimiter), m_rowsRead (0), m_fileStream (filepath), m_stream (&m_fileStream)
{
...
CsvReader::ParseColumn (std::string::const_iterator begin, std::string::const_iterator end)
{
  ...
  enum class State { BEGIN, END_QUOTE, FIND_DELIMITER, QUOTED_STRING, UNQUOTED_STRING, END };
