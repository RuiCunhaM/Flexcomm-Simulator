...
Ipv4ClickRouting::Ipv4ClickRouting () : m_nonDefaultName (false), m_ipv4 (0)
{
...
int
Ipv4ClickRouting::WriteHandler (std::string elementName, std::string handlerName,
                                std::string writeString)
{
  int r = simclick_click_write_handler (m_simNode, elementName.c_str (), handlerName.c_str (),
                                        writeString.c_str ());
...
Ptr<Ipv4Route>
Ipv4ClickRouting::RouteOutput (Ptr<Packet> p, const Ipv4Header &header, Ptr<NetDevice> oif,
...
// calls these methods.
int
simclick_sim_send (simclick_node_t *simnode, int ifid, int type, const unsigned char *data, int len,
                   simclick_simpacketinfo *pinfo)
{
  ...
  switch (cmd)
    {
      ...
      case SIMCLICK_GET_RANDOM_INT: {
        ...
        *randomValue = static_cast<uint32_t> (clickInstance->GetRandomVariable ()->GetValue (
